...
";
        ...
        public ContractModels getContract(string projectId, string contractid, string prid_s, string prid_e,string isWage)
        {
            ...
            c.planEST = form;
            //1.取得專案資料 
            log.Debug("get project by project_id=" + projectId);
            c.project = getProjectById(projectId);
            //2.取得估驗明細資料
            log.Debug("get EstimationOrder4Expense by project_id=" + contractid + "," + prid_s + "," + prid_e);
            switch (isWage)
            {
                case "Y":
            ...
            //3.取得供應商資料
            log.Debug("get Supplier by SUPPLIER_ID=" + c.EstimationItems.First().SUPPLIER_ID);
            c.supplier = getSupplierInfo(c.EstimationItems.First().SUPPLIER_ID);
            //4.取得代扣資料
            log.Debug("get Hold4DeductForm by SUPPLIER_ID=" + c.supplier.SUPPLIER_ID);
        ...
        public ContractModels getContract4TempWork(string projectId,string rptStartId,string rtpEndId,string supplierId,string chargeId)
        {
            ...
            log.Debug("get getContract4TempWork by project_id=" + projectId + "," + rptStartId + "," + rtpEndId);
...
";
            ...
            log.Info("get contract from Reaport=" + sql);
...
";
        ...
        private void caluateSumByContract(ContractModels contract)
        {
            ...
            if (payCond.PAYMENT_TYPE == "P")
            {
                ...
                {
                    log.Info("process for prepay info :");
                }
                else
                {
                    //預付款攤還金額計算
...
";
            log.Debug(sql + ",formId=" + f.EST_FORM_ID + ",Contract=" + f.CONTRACT_ID);
            var parameters = new List<SqlParameter>();
        ...
        public static void modifyEstimationInvoice(PLAN_ESTIMATION_FORM form, List<PLAN_ESTIMATION_INVOICE> lstInvoice, TopmepEntities context)
        {
            ...
            log.Debug("remove estimation invoice:sql=" + sql + ",formId=" + form.EST_FORM_ID);
            var parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("formId", form.EST_FORM_ID));
            context.Database.ExecuteSqlCommand(sql, parameters.ToArray());
            log.Debug("sql=" + sql + ",formId=" + form.EST_FORM_ID);
        ...
        public void delEstimationOrder(string formId)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("sql=" + sql + ",formId=" + formId);
                context.Database.ExecuteSqlCommand(sql, parameters.ToArray());
            }
        }
        //修改基本資料
...
";
        ...
        public List<EstimationForm> getContractItemById(string contractid, string projectid)
        {
            ...
            log.Info("get contract item by contractid  =" + contractid);
    ...
    public class InvoiceService
    {
        ...
        public decimal getTaxRate(string invoicetype)
        {
            ...
            if (idx > -1)
            {
                log.Debug("get tax Rate Type=" + invoicetype + ",Rate=" + TaxRate[idx]);
                return TaxRate[idx];
            }
            return -1;
        }
