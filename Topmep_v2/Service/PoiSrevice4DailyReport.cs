...
namespace Topmep.Service
{
    public class SummaryDailyReportToExcel : ExpenseFormToExcel
    {
        ...
        string expenseFile = ContextService.strTemplatePath + "\\dailyReport_Summary.xlsx";
        ...
        protected void writeHead()
        {
            ...
            log.Debug("Row 1 count:" + sheet.GetRow(1).Cells.Count());
            for (int i = 0; i < 21; i++)
            {
                log.Debug("Cell(" + i + ")=" + sheet.GetRow(1).GetCell(i));
            }
        ...
        protected void writeEstimateItem()
        {
            ...
            foreach (SummaryDailyReport dr in dailyReport)
            {
                ...
                {
                    log.Debug("Create Summary Row:" + iStartRow);
                    //補上小計資料
                ...
                {
                    //廠商資料
                    log.Debug("Create Supplier Row:" + iStartRow);
                    strSupplier = dr.SUPPLIER_ID;
                    rowInsert.CreateCell(0).SetCellValue(strSupplier);
                ...
                {
                    //log.Debug("Create Item Row:" + iStartRow);
                    log.Debug("Create Item Row :" + dr.PROJECT_ITEM_ID + " ,iRow=" + iStartRow);
            ...
            //寫入簽核欄位
            log.Debug("create row for approve field:" + iStartRow);
