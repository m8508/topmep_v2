...
namespace Topmep.Service
{
    ...
    public class ProjectTask2MapService : ProjectItemFromExcel
    {
        ...
        protected new List<PLAN_TASK2MAPITEM> ConverData2MapFW()
        {
            ...
            log.Info("MAP_FW Count:" + lstTask2MapFw.Count);
        ...
        private List<PLAN_TASK2MAPITEM> convertRow2TndMapFW(IRow row, int excelrow)
        {
            ...
            {
                log.Debug("create task map =" + row.Cells[7].ToString());
                ...
                for (int i = 0; i < strPrjUid.Length; i++)
                {
                    ...
                    log.Debug("create task map =" + item.PRJ_UID);
            ...
            {
                log.Warn("Data format maybe have error(TND_MAP_FW):" + excelrow);
            }
            log.Debug(excelrow + " had TND_MAP_FW Count:" + lstAllTask.Count);
            return lstAllTask;
        }
        #endregion
        #region 消防電資料轉換 
        ...
        protected new List<PLAN_TASK2MAPITEM> ConverData2MapFP()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                if (row.Cells.Count <10 && row.Cells[0].ToString().ToUpper() != "END")
                {
                    log.Debug("FP iRows=" + iRowIndex);
                    lstTask2MapFp.AddRange(convertRow2TndMapFP(row, iRowIndex));
                }
                iRowIndex++;
            }
            log.Info("MAP_FP Count:" + lstTask2MapFp.Count);
        ...
        private List<PLAN_TASK2MAPITEM> convertRow2TndMapFP(IRow row, int excelrow)
        {
            ...
            if (row.Cells[7].ToString().Trim() != "" && row.Cells[8].ToString().Trim() != "")//0.專案認識識別碼
            {
                log.Debug("crate task map =" + row.Cells[7].ToString());
                string[] strPrjUid = row.Cells[7].ToString().Split(',');
                for (int i = 0; i < strPrjUid.Length; i++)
                {
                    PLAN_TASK2MAPITEM item = new PLAN_TASK2MAPITEM();
            ...
            {
                log.Warn("Data format maybe have error(TND_MAP_FP):" + excelrow);
            }

            //消防電/管
            if (row.Cells[13].ToString().Trim() != "" && row.Cells[14].ToString().Trim() != "")//0.專案認識識別碼
            {
                log.Debug("crate task map =" + row.Cells[13].ToString());
            ...
            log.Debug(excelrow + " had TND_MAP_FP Count:" + lstItem.Count);
            return lstItem;
        }
        #endregion
        ...
        protected new List<PLAN_TASK2MAPITEM> ConverData2MapPEP()
        {
            ...
            {
                row = (IRow)rows.Current;
                log.Debug("PEP Excel Value:" + iRowIndex+ ","+ row.Cells.Count);
                if (row.Cells.Count > 10 && row.Cells[0].ToString().ToUpper() != "END")
                {
                    log.Debug("PEP iRows=" + iRowIndex);
                    lstTask2MapPep.AddRange(convertRow2TndMapPEP(row, iRowIndex));
                }
                iRowIndex++;
            }
            log.Info("MAP_PEP Count:" + lstTask2MapPep.Count);
        ...
        private List<PLAN_TASK2MAPITEM> convertRow2TndMapPEP(IRow row, int excelrow)
        {
            ...
            {
                log.Warn("Data format maybe have error(TND_MAP_PEP):" + excelrow);
            }
            ...
            {
                log.Debug("crate task map =" + row.Cells[17].ToString());
            ...
            log.Debug(excelrow + " had TND_MAP_PEP Count:" + lstItem.Count);
        ...
        protected new List<PLAN_TASK2MAPITEM> ConverData2MapPLU()
        {
            ...
            {
                row = (IRow)rows.Current;
                log.Debug("PLU Excel Value:" + iRowIndex + "," + row.Cells.Count);
            ...
            log.Info("MAP_PEP Count:" + lstTask2MapPlu.Count);
        ...
        private List<PLAN_TASK2MAPITEM> convertRow2TndMapPLU(IRow row, int excelrow)
        {
            ...
            {
                log.Warn("Data format maybe have error(TND_MAP_PLU):" + excelrow);
            }

            log.Debug(excelrow + " had TND_MAP_PLU Count:" + lstItem.Count);
