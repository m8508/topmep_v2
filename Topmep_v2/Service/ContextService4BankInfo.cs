...
namespace Topmep.Service
{
    ...
    public class ContextService4BankInfo : ContextService
    {
        ...
        public List<FIN_BANK_ACCOUNT> getAllBankAccount()
        {
            ...
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    log.Info("new bank account records=" + lstBankAccount.Count);
                }
                catch (Exception ex)
                {
                    log.Error(ex.Message + ":StackTrace=" + ex.StackTrace);
        ...
        public void updateBankAccount(List<FIN_BANK_ACCOUNT> lstBankAccount)
        {
            using (var context = new TopmepEntities())
            {
                ...
                foreach (FIN_BANK_ACCOUNT account in lstBankAccount)
                {
                    ...
                    log.Info("update bank account record=" + account.BANK_ACCOUNT_ID + ",initial amount=" + account.CUR_AMOUNT + ",curDate=" + account.CUR_DATE);
        ...
        public void getAllPlan()
        {
            ...
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    log.Debug("get records=" + lstPlans.Count);
        ...
        public List<BankLoanInfoExt> getAllSupplierLoan()
        {
            ...
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    log.Info("new supplier loan records=" + lstSupplierLoan.Count);
                }
                catch (Exception ex)
                {
                    log.Error(ex.Message + ":StackTrace=" + ex.StackTrace);
        ...
        {
            log.Debug("get bank loan transaction item by id=" + itemid);
