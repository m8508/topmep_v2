...
namespace Topmep.Service
{
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string budgetFile = ContextService.strTemplatePath + "\\budget_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        ...
        public string exportExcel(TND_PROJECT project)
        {
            ...
            {
                //取得直接成本作為預算初始值
                log.Info("Initial Budget Data for " + project.PROJECT_ID);
                typecodeItems = service.getDirectCost4Budget(project.PROJECT_ID);
            }
            else
            {
                //取得預算值填入
                log.Info("Get Budget Data for " + project.PROJECT_ID);
        ...
        public List<PLAN_BUDGET> ConvertDataForBudget(string projectId, SYS_USER u)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":預算");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("預算");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":預算");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("預算");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有預算資料(Sheet)! filename=" + fileformat);
        ...
        protected List<PLAN_BUDGET> ConverData2Budget()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                else
                {
                    ...
                    log.Info("Finish convert Job : count=" + lstBudget.Count);
                    return lstBudget;
                }
                iRowIndex++;
            }
            log.Info("Plan_Budget Count:" + iRowIndex);
            return lstBudget;
        }
        /**
        ...
        private PLAN_BUDGET convertRow2PlanBudget(IRow row, int excelrow)
        {
            ...
            if (row.Cells[3].ToString().Trim() != "")//3,合約金額
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[3].ToString());
                    item.CONTRACT_AMOUNT = contractAmt;
                }
                catch (Exception e)
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[6].value=" + row.Cells[row.Cells.Count - 6].ToString());
            ...
            if (row.Cells[4].ToString().Trim() != "")//4.材料成本
            {
                ...
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[4].value=" + row.Cells[4].ToString());
                    log.Error(e.StackTrace);
                }
            }
            if (row.Cells[5].ToString().Trim() != "")//5材料折扣
            {
                try
                {
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[5].ToString());
                    decimal ratio = decimal.Parse(row.Cells[5].ToString());
                    item.BUDGET_RATIO = ratio;
                }
                catch (Exception e)
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[6].value=" + row.Cells[6].ToString());
                    log.Error(e.StackTrace);
                }
            }
            if (row.Cells[7].ToString().Trim() != "")//7.工資成本,
            {
                try
                {
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[7].ToString());
                    decimal amount = decimal.Parse(row.Cells[7].ToString());
                    item.BUDGET_WAGE_AMOUNT = amount;
                }
                catch (Exception e)
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[7].value=" + row.Cells[7].ToString());
                    log.Error(e.StackTrace);
                }
            }
            if (row.Cells[8].ToString().Trim() != "")//8.工資折扣,
            {
                ...
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[8].value=" + row.Cells[8].ToString());
                    log.Error(e.StackTrace);
                }
            }
            ...
            log.Info("PLAN_BUDGET=" + item.ToString());
            return item;
        }
        #endregion
    ...
    public class PurchaseFormtoExcel
    {
        ...
        string templateFile4All = ContextService.strTemplatePath + "\\Inquiry_form_templateAll.xlsx";
        ...
        public void convertInquiry2Plan(string fileExcel, string projectid, string iswage)
        {
            ...
            while (hasMore)
            {
                ...
                log.Info("excel rowid=" + iRowIndex + ",cell count=" + row.Cells.Count);
                ...
                else
                {
                    try
                    {
                        log.Debug("row id=" + iRowIndex + "Cells Count=" + row.Cells.Count + ",purchase form item vllue:" + row.Cells[0].ToString() + ","
                        ...
                        else
                        {
                            ...
                            {
                                log.Warn("RowId=" + iRowIndex + " not have Qty!!" + ex.StackTrace);
                            }
                            ...
                            try
                            {
                                ...
                                log.Warn("RowId=" + iRowIndex + "," + item.ITEM_DESC + ", Unprice=" + dUnitPrice);
                                item.ITEM_UNIT_PRICE = dUnitPrice;
                            }
                            catch (Exception ex)
                            {
                                log.Warn("RowId=" + iRowIndex + " not have Unit price!!" + ex.StackTrace);
                            }

                            item.ITEM_REMARK = row.Cells[6].ToString();
                            log.Info("Plan ITEM ID=" + row.Cells[row.Cells.Count - 1].ToString());
            ...
            while (hasMore)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            log.Warn("Row Index=" + iRowIndex + "item.ITEM_QTY Error:" + ex.StackTrace);
                        }
                        //材料單價
                        try
                        {
                        ...
                        {
                            log.Warn("Row Index=" + iRowIndex + "item.ITEM_UNIT_PRICE Error:" + ex.StackTrace);
                        }
                        //材料單價
                        try
                        {
                        ...
                        {
                            log.Warn("Row Index=" + iRowIndex + "item.WAGE_PRICE Error:" + ex.StackTrace);
                        }
        ...
        private void processForm(string projectid, string iswage)
        {
            ...
            try
            {
                log.Debug(sheet.GetRow(7).Cells[1].ToString());
                form.OWNER_FAX = sheet.GetRow(7).Cells[1].ToString();
            }
            catch (Exception ex)
            {
                form.OWNER_FAX = "";
                log.Error("FAX Null :" + ex.StackTrace);
            }
        }
        //2.填入表頭資料
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string budgetFile = ContextService.strTemplatePath + "\\expense_budget_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        IWorkbook hssfworkbook;
        ISheet sheet = null;
        string fileformat = "xlsx";
        ...
        public string exportExcel()
        {
            ...
            foreach (FIN_SUBJECT item in subjects)
            {
                ...
                log.Debug("getSubject cell style rowid=" + idxRow);
                idxRow++;
            }
            //4.另存新檔至專案所屬目錄 (增加Temp for zip 打包使用
            string fileLocation = null;
        ...
        public List<FIN_EXPENSE_BUDGET> ConvertDataForExpBudget(int year)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for Year=" + year + ":公司費用預算");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("公司費用預算");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for Year=" + year + ":預算");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("公司費用預算");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有公司費用預算資料(Sheet)! filename=" + fileformat);
        ...
        protected List<FIN_EXPENSE_BUDGET> ConverData2ExpBudget()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                else
                {
                    ...
                    log.Info("Finish convert Job : count=" + subjects.Count);
                    return lstExpBudget;
                }
                iRowIndex++;
            }
            log.Info("Expense_Budget Count:" + iRowIndex);
            return lstExpBudget;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    if (null != row.Cells[i + 2].ToString().Trim() || row.Cells[i + 2].ToString().Trim() != "")
                    {
                        try
                        {
                            ...
                            log.Info("excelrow=" + excelrow + ",value=" + row.Cells[i + 2].ToString());
                            item.AMOUNT = dAmt;
                        }
                        catch (Exception e)
                        {
                            log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[i+2].value=" + row.Cells[i + 2].ToString());
                            log.Error(e);
                        }
                    }
                    lst.Add(item);
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string expenseFile = ContextService.strTemplatePath + "\\expense_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        public IWorkbook hssfworkbook;
        ISheet sheet = null;
        ...
        public string exportExcel(ExpenseFormFunction ExpTable, List<ExpenseBudgetSummary> EXPTableItem, List<ExpenseBudgetSummary> SiteTableItem, ExpenseBudgetSummary ExpAmt, ExpenseBudgetSummary EarlyExpAmt, ExpenseBudgetSummary siteEarlyExpAmt)
        {
            ...
            {
                float page = 1;
                log.Info("工地費用單頁數 =" + Convert.ToInt16(Math.Ceiling(float.Parse(EXPTableItem.Count.ToString()) / 47)));
                ...
                if (float.Parse(EXPTableItem.Count.ToString()) / 47 > 1)
                {
                    log.Debug("Table Head_6=" + sheet.GetRow(86).Cells[1].ToString());
                    sheet.GetRow(86).Cells[1].SetCellValue(ExpTable.REMARK);//說明事項
                    log.Debug("Table Head_7=" + sheet.GetRow(86).Cells[12].ToString());
                    ...
                    log.Debug("Table Head_8=" + sheet.GetRow(89).Cells[12].ToString());
                ...
                {
                    log.Debug("Table Head_6=" + sheet.GetRow(40).Cells[1].ToString());
                    sheet.GetRow(40).Cells[1].SetCellValue(ExpTable.REMARK);//說明事項
                    log.Debug("Table Head_7=" + sheet.GetRow(40).Cells[12].ToString());
                    if (siteBudget.TOTAL_BUDGET.ToString() != "")
                    {
                        sheet.GetRow(40).Cells[13].SetCellValue(double.Parse(siteBudget.TOTAL_BUDGET.ToString()));//總預算
                    }
                    log.Debug("Table Head_8=" + sheet.GetRow(43).Cells[12].ToString());
                    if (siteEarlyExpAmt.AMOUNT.ToString() != "")
                    {
                    ...
                    log.Debug("Table Head_9=" + sheet.GetRow(44).Cells[12].ToString());
                    sheet.GetRow(44).Cells[13].SetCellValue(double.Parse(ExpAmt.AMOUNT.ToString()));//本期金額
                    log.Debug("Table Head_10=" + sheet.GetRow(45).Cells[12].ToString());                                                                              //工資預算
                    sheet.GetRow(45).Cells[13].CellFormula = "(N44+N45)";
                    log.Debug("Table Head_11=" + sheet.GetRow(46).Cells[12].ToString());
                    sheet.GetRow(46).Cells[13].SetCellValue(Convert.ToDateTime(ExpTable.PAYMENT_DATE).ToShortDateString());//支付日期
                    log.Debug("Table Head_12=" + sheet.GetRow(27).Cells[7].ToString());
                ...
                foreach (ExpenseBudgetSummary item in SiteTableItem)
                {
                    ...
                    //品名或摘要
                    log.Debug("ITEM_REMARK=" + item.ITEM_REMARK);
                    ...
                    //會計科目名稱
                    log.Debug("SUBJECT_NAM=" + item.SUBJECT_NAME);
                    row.Cells[20].SetCellValue(item.SUBJECT_NAME);
                    log.Debug("get Site Expense Form cell style rowid=" + idxRow);
                    if (idxRow == 47)
                    {
                        idxRow = idxRow + 4;
                    }
            ...
            {
                float page = 1;
                log.Info("公司費用單頁數 =" + Convert.ToInt16(Math.Ceiling(float.Parse(EXPTableItem.Count.ToString()) / 47)));
                if (float.Parse(EXPTableItem.Count.ToString()) / 47 > 1)
                {
                    page = float.Parse(EXPTableItem.Count.ToString()) / 47;
                    sheet = (XSSFSheet)hssfworkbook.GetSheet("費用表_2頁");
                ...
                if (float.Parse(EXPTableItem.Count.ToString()) / 47 > 1)
                {
                    ...
                    log.Debug("Table Head_9=" + sheet.GetRow(90).Cells[12].ToString());
                    sheet.GetRow(90).Cells[13].SetCellValue(double.Parse(ExpAmt.AMOUNT.ToString()));//本期金額
                    log.Debug("Table Head_10=" + sheet.GetRow(91).Cells[12].ToString());                                                                              //工資預算
                ...
                foreach (ExpenseBudgetSummary item in EXPTableItem)
                {
                    ...
                    log.Debug("SUBJECT_NAM=" + item.SUBJECT_NAME);
                    row.Cells[20].SetCellValue(item.SUBJECT_NAME);
                    log.Debug("get Expense Form cell style rowid=" + idxRow);
                    if (idxRow == 47)
                    {
                        idxRow = idxRow + 4;
                    }
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string summaryFile = ContextService.strTemplatePath + "\\ExpBudgetSummary_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        IWorkbook hssfworkbook;
        ISheet sheet = null;
        XSSFCellStyle style = null;
        ...
        public string exportExcel(int year)
        {
            ...
            {
                log.Info("Budget Row Id=" + idxRow);
                createRow(idxRow, item, "預算數");
                idxRow++;
                //由會計科目取得實際數資料
                log.Info("Execute Row Id=" + idxRow);
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string orderFile = ContextService.strTemplatePath + "\\MaterialOrder_form.xlsx";
        string deliveryFile = ContextService.strTemplatePath + "\\MaterialDelivery_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        IWorkbook hssfworkbook;
        ISheet sheet = null;
        XSSFCellStyle style = null;
        ...
        public string exportExcel(TND_PROJECT project, PLAN_PURCHASE_REQUISITION tablePR, List<PurchaseRequisition> orderItem, List<PurchaseRequisition> deliveryItem, bool isOrder, bool isDO)
        {
            ...
            if (isOrder)
            {
                ...
                log.Debug("Table Head_3=" + sheet.GetRow(3).Cells[0].ToString());
                sheet.GetRow(3).Cells[2].SetCellValue(tablePR.PR_ID);//採購單號
                log.Debug("Table Head_4=" + sheet.GetRow(5).Cells[0].ToString());
                sheet.GetRow(5).Cells[2].SetCellValue(tablePR.RECIPIENT);//收件人
                log.Debug("Table Head_5=" + sheet.GetRow(6).Cells[0].ToString());
                sheet.GetRow(6).Cells[2].SetCellValue(tablePR.LOCATION);//送貨地址
                log.Debug("Table Head_6=" + sheet.GetRow(7).Cells[0].ToString());
            ...
            else
            {
                ...
                log.Debug("Table Head_4=" + sheet.GetRow(4).Cells[0].ToString());
                ...
                if (isDO)
                {
                    ...
                    log.Debug("Table Head_0=" + sheet.GetRow(0).Cells[2].ToString());
                    sheet.GetRow(0).Cells[3].SetCellValue("領料單");
                    log.Debug("Table Head_5=" + sheet.GetRow(6).Cells[4].ToString());
            ...
            if (isOrder)
            {
                ...
                foreach (PurchaseRequisition item in orderItem)
                {
                    ...
                    log.Debug("get Material Order Form cell style rowid=" + idxRow);
                    idxRow++;
                }
                idxRow++;
                IRow nextRow = sheet.CreateRow(idxRow);//.GetRow(idxRow);
                log.Info("Next Row Id=" + idxRow);
            ...
            else if (isDO)
            {
                ...
                foreach (PurchaseRequisition item in deliveryItem)
                {
                    ...
                    row.Cells[5].CellStyle = style;
                    log.Debug("get Material DO Form cell style rowid=" + idxRow);
                    idxRow++;
                }
                idxRow++;
                IRow newRow = sheet.CreateRow(idxRow);//.GetRow(idxRow);
                log.Info("New Row Id=" + idxRow);
                newRow.CreateCell(0);
            ...
            else
            {
                ...
                foreach (PurchaseRequisition item in orderItem)
                {
                    ...
                    row.Cells[5].CellStyle = style;
                    log.Debug("get Material DF Form cell style rowid=" + idxRow);
                    idxRow++;
                }
                idxRow++;
                IRow newRow = sheet.CreateRow(idxRow);//.GetRow(idxRow);
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string budgetFile = ContextService.strTemplatePath + "\\site_budget_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        IWorkbook hssfworkbook;
        ISheet sheet = null;
        string fileformat = "xlsx";
        ...
        public List<PLAN_SITE_BUDGET> ConvertDataForSiteBudget1(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + "工地費用預算_第1年度");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工地費用預算_第1年度");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + "工地費用預算_第1年度");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("工地費用預算_第1年度");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有:工地費用預算_第1年度(Sheet)! filename=" + fileformat);
        ...
        public List<PLAN_SITE_BUDGET> ConvertDataForSiteBudget2(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + "工地費用預算_第2年度");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工地費用預算_第2年度");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + "工地費用預算_第2年度");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("工地費用預算_第2年度");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有:工地費用預算_第2度(Sheet)! filename=" + fileformat);
        ...
        public List<PLAN_SITE_BUDGET> ConvertDataForSiteBudget3(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + "工地費用預算_第3年度");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工地費用預算_第3年度");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + "工地費用預算_第3年度");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("工地費用預算_第3年度");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有:工地費用預算_第3年度(Sheet)! filename=" + fileformat);
        ...
        public List<PLAN_SITE_BUDGET> ConvertDataForSiteBudget4(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + "工地費用預算_第4年度");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工地費用預算_第4年度");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + "工地費用預算_第4年度");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("工地費用預算_第4年度");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有:工地費用預算_第4年度(Sheet)! filename=" + fileformat);
        ...
        public List<PLAN_SITE_BUDGET> ConvertDataForSiteBudget5(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + "工地費用預算_第5年度");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工地費用預算_第5年度");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + "工地費用預算_第5年度");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("工地費用預算_第5年度");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有:工地費用預算_第5年度(Sheet)! filename=" + fileformat);
        ...
        protected List<PLAN_SITE_BUDGET> ConverData2SiteBudget()
        {
            ...
            {
                log.Error("The Year Info is empty!!");
            ...
            log.Info("Plan_Site_Budget Count:" + iRowIndex);
            return lstSiteBudget;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string summaryFile = ContextService.strTemplatePath + "\\SiteExpSummary_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        IWorkbook hssfworkbook;
        ISheet sheet = null;
        XSSFCellStyle style = null;
        ...
        public string exportExcel(string projectid, string projectName)
        {
            ...
            foreach (ExpenseBudgetSummary sitebudgetyear in SiteBudgetYears)
            {
                ...
                foreach (ExpenseBudgetSummary item in SiteBudget)
                {
                    ...
                    log.Debug("Fill Site Budget cell style rowid=" + idxRow);
                    idxRow++;
                    //4.填入實際數
                    ...
                    //項目、項目代碼、預算/費用、1月、2月、3月、4月、5月、6月、7月、8月、9月、10月、11月、12月、合計
                    //項目
                    log.Debug("SUBJECT_NAM=" + exp.SUBJECT_NAME + ",Subject_Id=" + exp.SUBJECT_ID);
                    row.CreateCell(0).SetCellValue("");
                    row.Cells[0].CellStyle = style;
                    //項目代碼
                    row.CreateCell(1).SetCellValue("");
                    ...
                    log.Debug("Fill Site Expense rowid=" + idxRow);
                    idxRow++;
                }
            }
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string creditNoteFile = ContextService.strTemplatePath + "\\credit_note_form.xlsx";
        string outputPath = ContextService.strUploadPath;
        ...
        public string exportExcel(List<CreditNote> CreditNoteItem, RevenueFromOwner va)
        {
            ...
            foreach (CreditNote item in CreditNoteItem)
            {
                ...
                row.Cells[8].SetCellValue(item.INVOICE_NUMBER.Substring(2, 8));//號碼
                                                                               //品名
                log.Debug("PLAN_ITEM_ID=" + item.PLAN_ITEM_ID);
                ...
                row = sheet.GetRow(idxRow);
                //買受人
                log.Debug("OWNER_NAME=" + item.OWNER_NAME);
                ...
                log.Debug("get credit note cell style rowid=" + idxRow);
                i++;
            }
