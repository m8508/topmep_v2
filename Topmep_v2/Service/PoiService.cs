...
namespace Topmep.Service
{
    ...
    public class ExcelBase
    {
        ...
        public static string strUploadPath = ContextService.strUploadPath;
        public static string strTemplatePath = ContextService.strTemplatePath;
        ...
        public void InitializeWorkbook()
        {
            ...
            {
                log.Info("Read Excel File:" + templateFile);
                if (file.Name.EndsWith(".xls"))
                {
                    log.Debug("process excel file for office 2003");
                    fileformat = "xls";
                    hssfworkbook = new HSSFWorkbook(file);
                }
                else
                {
                    log.Debug("process excel file for office 2007");
        ...
        public void InitializeWorkbook(string path)
        {
            ...
            {
                log.Info("Read Excel File:" + path); if (file.Name.EndsWith(".xls"))
                {
                    log.Debug("process excel file for office 2003");
                    fileformat = "xls";
                    hssfworkbook = new HSSFWorkbook(file);
        ...
        public void SetOpSheet(string sheetName)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat);
                sheet = (HSSFSheet)hssfworkbook.GetSheet(sheetName);
            }
            else
            {
                log.Debug("office 2007:" + fileformat);
                sheet = (XSSFSheet)hssfworkbook.GetSheet(sheetName);
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有Sheet=" + sheetName + "! filename=" + fileformat);
    ...
    public class ProjectItemFromExcel
    {
        ...
        public void ConvertDataForTenderProject(string projectId, int startrow)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId);
                sheet = (HSSFSheet)hssfworkbook.GetSheet("整理後標單");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId);
                sheet = (XSSFSheet)hssfworkbook.GetSheet("整理後標單");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有整理後標單資料(Sheet)! filename=" + fileformat);
        ...
        public void ConvertExcelToTndProjectItem(int startrow)
        {
            ...
            while (iRowIndex < (startrow - 1))
            {
                ...
                if (null == row)
                {
                    log.Debug("Empty Row Index=" + iRowIndex);
                }
                else if (row.Cells.Count > 2)
                {
                    log.Debug("skip data Excel Value:" + iRowIndex + ","
            ...
            while (hasMore)
            {
                ...
                try
                {
                    if (row != null)
                    {
                        ...
                        log.Debug("Cells Count=" + row.Cells.Count + ",Excel Index:" + iRowIndex);
                    }
                    else
                    {
                        log.Warn("empty row:" + iRowIndex);
                    ...
                    {
                        log.Info("Finish convert Job : count=" + lstProjectItem.Count);
                        hasMore = false;
                        return;
                    }
                    else
                ...
                {
                    log.Error(iRowIndex + ",ex:" + ex.Message + "," + ex.StackTrace);
                }
                iRowIndex++;
                itemId++;
            }
        ...
        private TND_PROJECT_ITEM convertRow2TndProjectItem(int id, IRow row, int excelrow)
        {
            ...
            log.Debug("irow=" + excelrow + ",Row=" + row + "," + row.Count());
            ...
            if (row.GetCell(3) != null && row.GetCell(3).ToString().Trim() != "")//數量
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.GetCell(3).ToString());
                    projectItem.ITEM_QUANTITY = dQty;
                }
                catch (Exception e)
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Item_Desc= " + projectItem.ITEM_DESC + ",value=" + row.GetCell(3).ToString());
            ...
            log.Info("TndprojectItem=" + projectItem.ToString());
            return projectItem;
        }
        ...
        public void ConvertDataForPlan(string projectId)
        {
            ...
            if (null == sheet)
            {
                log.Error("檔案內沒有得標後標單資料(Sheet)! filename=" + fileformat);
        ...
        public void ConvertExcelToPlanItem()
        {
            ...
            while (iRowIndex < 4)
            {
                ...
                log.Debug("skip data Excel Value:" + row.Cells[0].ToString() + "," + row.Cells[1] + "," + row.Cells[2]);
            }
            //循序處理每一筆資料之欄位!!
            iRowIndex++;
            int itemId = 1;
            ...
            {
                row = (IRow)rows.Current;
                log.Debug("Cells Count=" + row.Cells.Count + ",Excel Value:" + row.Cells[0].ToString());
                //將各Row 資料寫入物件內
                ...
                {
                    log.Info("Finish convert Job : count=" + lstPlanItem.Count);
                    return;
                }
                iRowIndex++;
                itemId++;
        ...
        {
            log.Debug("Cells Count=" + row.Cells.Count + ",Row PhysicalNumberOfCells no=:" + row.PhysicalNumberOfCells);
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                try
                {
                    log.Debug("for feaure rowID=" + id + ",cell style=" + row.GetCell(i).CellType);
                }
                catch (Exception ex)
                {
                    log.Warn("for feaure rowID=" + id + ",cell tyle=" + i + "ex:" + ex.Message);
                }
                    ...
                    switch (i)
                    {
                        ...
                        case 4:
                            try
                            {
                                ...
                                log.Info("excelrow=" + excelrow + ",value=" + cell.ToString());
                                planItem.ITEM_QUANTITY = dQty;
                            }
                            catch (Exception e)
                            {
                                log.Error("data format Error on PLAN_ITEM_ID= " + planItem.PLAN_ITEM_ID + ",ExcelRow=" + excelrow + ",Item_Desc= " + planItem.ITEM_DESC + ",value=" + cell.ToString());
                        ...
                        case 6://項目標單單價
                            ...
                            {
                                decimal dQty = decimal.Parse(cell.ToString());
                                log.Info("excelrow=" + excelrow + ",value=" + dQty);
                        ...
                        case 8://備註
                            ...
                            break;
                        case 9:
                            log.Debug(id + " i index=" + i + "=" + cell.ToString());
        ...
        public List<TND_MAP_FW> ConvertDataForMapFW(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":消防水");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("消防水");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":消防水");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("消防水");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有消防水資料(Sheet)! filename=" + fileformat);
        ...
        protected List<TND_MAP_FW> ConverData2MapFW()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                {
                    log.Debug("ROW:" + iRowIndex + ",Excel Value:" + row.Cells[0].ToString() + row.Cells[1] + row.Cells[2]);
                    lstMapFW.Add(convertRow2TndMapFW(row, iRowIndex));
                }
                else
                {
                    logErrorMessage("Step1 ;取得圖算數量消防水:" + lstMapFW.Count + "筆");
                    log.Info("Finish convert Job : count=" + lstMapFW.Count);
                    return lstMapFW;
                }
                iRowIndex++;
            }
            log.Info("MAP_FW Count:" + iRowIndex);
            return lstMapFW;
        }
        /**
        ...
        {
            log.Debug("get last row no=" + row.LastCellNum);
            ...
            if (null != row.Cells[8].ToString().Trim() || row.Cells[8].ToString().Trim() != "")// 8管數/組
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[8].ToString());
                    item.PIPE_CNT = dQty;
                }
                catch (Exception e)
                {
            ...
            if (null != row.Cells[9].ToString().Trim() || row.Cells[9].ToString().Trim() != "")// 9.管組數	
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[9].ToString());
                    item.PIPE_SET = dQty;
                }
                catch (Exception e)
                {
            ...
            if (null != row.Cells[10].ToString().Trim() || row.Cells[10].ToString().Trim() != "")// 10管長度/組數
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[10].ToString());
            ...
            if (null != row.Cells[11].ToString().Trim() || row.Cells[11].ToString().Trim() != "")// 11管總長
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[11].ToString());
            ...
            log.Info("TND_MAP_FW=" + item.ToString());
            return item;
        }
        #endregion
        #region 消防電資料轉換 
        ...
        public List<TND_MAP_FP> ConvertDataForMapFP(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":消防電");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("消防電");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":消防電");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("消防電");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有消防電資料(Sheet)! filename=" + fileformat);
        ...
        protected List<TND_MAP_FP> ConverData2MapFP()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                if (row.Cells.Count != 0 && row.Cells[0].ToString().ToUpper() != "END")
                {
                    log.Debug("Excel Value:" + row.Cells[0].ToString() + row.Cells[1] + row.Cells[2]);
                    lstMapFP.Add(convertRow2TndMapFP(row, iRowIndex));
                }
                else
                {
                    logErrorMessage("Step1 ;取得圖算數量消防電:" + lstMapFP.Count + "筆");
                    log.Info("Finish convert Job : count=" + lstMapFP.Count);
                    return lstMapFP;
                }
                iRowIndex++;
            }
            log.Info("MAP_FP Count:" + iRowIndex);
            return lstMapFP;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
        ...
        private TND_MAP_FP convertRow2TndMapFP(IRow row, int excelrow)
        {
            ...
            if (row.Cells[13].ToString().Trim() != "")//13.管長
            {
                ...
                {
                    decimal dQty = decimal.Parse(row.Cells[10].ToString());
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[13].ToString());
                    item.PIPE_LENGTH = dQty;
                }
                catch (Exception e)
                {
            ...
            if (row.Cells[14].ToString().Trim() != "")//14.管組數	
            {
                ...
                {
                    decimal dQty = decimal.Parse(row.Cells[11].ToString());
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[14].ToString());
                    item.PIPE_SET = dQty;
                }
                catch (Exception e)
                {
            ...
            if (row.Cells[15].ToString().Trim() != "")//15.管組數	
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[15].ToString());
                    item.PIPE_TOTAL_LENGTH = dQty;
                }
                catch (Exception e)
                {
            ...
            item.CREATE_DATE = System.DateTime.Now;
            log.Info("TND_MAP_FP=" + item.ToString());
            return item;
        }
        #endregion
        #region 清單設備資料轉換 
        ...
        public List<TND_MAP_DEVICE> ConvertDataForMapDEVICE(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":工率");//因為與工率共用一個EXCEL SHEET
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工率");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":工率");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("工率");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有設備清單資料(Sheet)! filename=" + fileformat);
        ...
        protected List<TND_MAP_DEVICE> ConverData2MapDEVICE()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                else
                {
                    ...
                    log.Info("Finish convert Job : count=" + lstMapDEVICE.Count);
                    return lstMapDEVICE;
                }
                iRowIndex++;
            }
            log.Info("MAP_DEVICE Count:" + iRowIndex);
            return lstMapDEVICE;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
        ...
        private TND_MAP_DEVICE convertRow2TndMapDEVICE(IRow row, int excelrow)
        {
            ...
            if (null != CellQty)//1.設備數量
            {
                ...
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[11].value=" + CellQty.ToString());
                    log.Error(e);
                }
            }
            item.CREATE_DATE = System.DateTime.Now;
            log.Info("TND_MAP_DEVICE=" + item.ToString());
            return item;
        }
        #endregion
        #region 給排水資料轉換 
        ...
        public List<TND_MAP_PLU> ConvertDataForMapPLU(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":給排水");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("給排水");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":給排水");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("給排水");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有給排水資料(Sheet)! filename=" + fileformat);
        ...
        protected List<TND_MAP_PLU> ConverData2MapPLU()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                if (row.Cells.Count != 0 && row.Cells[0].ToString().ToUpper() != "END")
                {
                    log.Debug("Excel Value:" + row.Cells[0].ToString());
                ...
                else
                {
                    ...
                    log.Info("Finish convert Job : count=" + lstMapPLU.Count);
                    return lstMapPLU;
                }
                iRowIndex++;
            }
            log.Info("MAP_PLU Count:" + iRowIndex);
            return lstMapPLU;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
        ...
        private TND_MAP_PLU convertRow2TndMapPLU(IRow row, int excelrow)
        {
            ...
            item.PROJECT_ID = projId;
            log.Debug("cell count:" + row.PhysicalNumberOfCells + ",Row:" + excelrow);
            try
            {
                ...
                item.CREATE_DATE = System.DateTime.Now;
                log.Info("TND_MAP_PLU=" + item.ToString());
                return item;
                #endregion
            }
            catch (Exception ex)
            {
                log.Debug("Row:" + excelrow + "cell count error :" + row.PhysicalNumberOfCells + ",ex=" + ex.StackTrace);
                return null;
            }
        }
        #endregion
        ...
        public List<TND_MAP_LCP> ConvertDataForMapLCP(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":弱電管線");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("弱電管線");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":弱電管線");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("弱電管線");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有弱電管線資料(Sheet)! filename=" + fileformat);
        ...
        protected List<TND_MAP_LCP> ConverData2MapLCP()
        {
            ...
            {
                row = (IRow)rows.Current;
                log.Debug("Excel Row=" + iRowIndex + ",Excel Value:" + row.Cells[0].ToString());
                //將各Row 資料寫入物件內
                ...
                if (row.Cells.Count != 0 && row.Cells[0].ToString().ToUpper() != "END")
                {
                    log.Debug("Excel Value:" + row.Cells[0].ToString() + row.Cells[1]);
                    lstMapLCP.Add(convertRow2TndMapLCP(row, iRowIndex));
                }
                else
                {
                    logErrorMessage("Step1 ;取得圖算數量弱電管線:" + lstMapLCP.Count + "筆");
                    log.Info("Finish convert Job : count=" + lstMapLCP.Count);
                    return lstMapLCP;
                }
                iRowIndex++;
            }
            log.Info("MAP_LCP Count:" + iRowIndex);
            return lstMapLCP;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
        ...
        private TND_MAP_LCP convertRow2TndMapLCP(IRow row, int excelrow)
        {
            ...
            {
                log.Debug("Excel Row:" + excelrow + ",MaxCol=" + iCol);
                    ...
                    switch (iCol)
                    {
                        ...
                        case 15://15管總長
                            ...
                            catch (FormatException fe)
                            {
                                ...
                                log.Error(fe.Message + "," + fe.StackTrace);
                            }
                            catch (Exception e)
                            {
                                log.Error(e.Message + "," + e.StackTrace);
            ...
            item.CREATE_DATE = System.DateTime.Now;
            log.Info("TND_MAP_LCP=" + item.ToString());
            return item;
        }
        #endregion
        #region 電氣管線資料轉換 
        ...
        public List<TND_MAP_PEP> ConvertDataForMapPEP(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":電氣管線");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("電氣管線");
            }
            else
            {
                log.Debug("office 2007:" + fileformat + " for projectID=" + projId + ":電氣管線");
                sheet = (XSSFSheet)hssfworkbook.GetSheet("電氣管線");
            }
            if (null == sheet)
            {
                log.Error("檔案內沒有電氣管線資料(Sheet)! filename=" + fileformat);
        ...
        protected List<TND_MAP_PEP> ConverData2MapPEP()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                else
                {
                    ...
                    log.Info("Finish convert Job : count=" + lstMapPEP.Count);
                    return lstMapPEP;
                }
                iRowIndex++;
            }
            log.Info("MAP_PEP Count:" + iRowIndex);
            return lstMapPEP;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
        ...
        private TND_MAP_PEP convertRow2TndMapPEP(IRow row, int excelrow)
        {
            ...
            if (row.Cells[16].ToString().Trim() != "")//16.管長
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[16].ToString());
                    item.PIPE_LENGTH = dQty;
                }
                catch (Exception e)
                {
            ...
            if (row.Cells[17].ToString().Trim() != "")//17.管組數
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[17].ToString());
                    item.PIPE_SET = dQty;
                }
                catch (Exception e)
                {
            ...
            if (row.Cells[18].ToString().Trim() != "")//18.管總長
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[18].ToString());
                    item.PIPE_TOTAL_LENGTH = dQty;
                }
                catch (Exception e)
                {
            ...
            item.CREATE_DATE = System.DateTime.Now;
            log.Info("TND_MAP_PEP=" + item.ToString());
            return item;
        }
        #endregion
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string wageFile = ContextService.strTemplatePath + "\\wage_form.xlsx";
        ...
        public void exportExcel(TND_PROJECT project, List<PROJECT_ITEM_WITH_WAGE> projectItems)
        {
            ...
            //2.填入表頭資料
            log.Debug("Table Head_1=" + sheet.GetRow(1).Cells[0].ToString());
            sheet.GetRow(1).Cells[1].SetCellValue(project.PROJECT_ID);//專案編號
            log.Debug("Table Head_2=" + sheet.GetRow(2).Cells[0].ToString());
            ...
            foreach (PROJECT_ITEM_WITH_WAGE item in projectItems)
            {
                ...
                log.Debug("Wage :ITEM DESC=" + item.ITEM_DESC);
        ...
        public List<TND_WAGE> ConvertDataForWage(string projectId)
        {
            ...
            if (fileformat == "xls")
            {
                log.Debug("office 2003:" + fileformat + " for projectID=" + projId + ":工率");
                sheet = (HSSFSheet)hssfworkbook.GetSheet("工率");
            }
            else
            {
            ...
            if (null == sheet)
            {
                log.Error("檔案內沒有工率資料(Sheet)! filename=" + fileformat);
                throw new Exception("檔案內沒有[工率]資料");
            }
        ...
        protected List<TND_WAGE> ConverData2Wage()
        {
            ...
            while (rows.MoveNext())
            {
                ...
                else
                {
                    ...
                    //log.Info("Finish convert Job : count=" + projectItems.Count);
                    return lstWage;
                }
                iRowIndex++;
            }
            log.Info("MAP_WAGE Count:" + iRowIndex);
            return lstWage;
        }
        /**
         * 將Excel Row 轉換成為對應的資料物件
        ...
        private TND_WAGE convertRow2TndWage(IRow row, int excelrow)
        {
            ...
            if (null != row.Cells[row.Cells.Count - 1].ToString().Trim() || row.Cells[row.Cells.Count - 1].ToString().Trim() != "")//1.工率
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[row.Cells.Count - 1].ToString());
                    item.RATIO = dQty;
                }
                catch (Exception e)
                {
                    log.Error("data format Error on ExcelRow=" + excelrow + ",Cells[12].value=" + row.Cells[row.Cells.Count - 1].ToString());
                    log.Error(e);
                }
            }
            item.CREATE_DATE = System.DateTime.Now;
            log.Info("TND_WAGE=" + item.ToString());
            return item;
        }
    ...
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string templateFile = ContextService.strTemplatePath + "\\Inquiry_form_template.xlsx";
        ...
        public string exportExcel(TND_PROJECT_FORM form, List<TND_PROJECT_FORM_ITEM> formItems, bool isTemp)
        {
            ...
            log.Debug("Template Head_1=" + sheet.GetRow(2).Cells[0].ToString());
            sheet.GetRow(2).Cells[1].SetCellValue(p.PROJECT_NAME);//專案名稱
            log.Debug("Template Head_2=" + sheet.GetRow(3).Cells[0].ToString());
            sheet.GetRow(3).Cells[1].SetCellValue(form.FORM_NAME);//採購項目:
            log.Debug("Template Head_3=" + sheet.GetRow(4).Cells[0].ToString());
            sheet.GetRow(4).Cells[1].SetCellValue(form.OWNER_NAME);//承辦人:
            log.Debug("Template Head_4=" + sheet.GetRow(5).Cells[0].ToString());
            sheet.GetRow(5).Cells[1].SetCellValue(form.OWNER_TEL);//聯絡電話:
            log.Debug("Template Head_5=" + sheet.GetRow(6).Cells[0].ToString());
            sheet.GetRow(6).Cells[1].SetCellValue(form.OWNER_EMAIL);//EMAIL:
            log.Debug("Template Head_6=" + sheet.GetRow(7).Cells[0].ToString());
            sheet.GetRow(7).Cells[1].SetCellValue(form.OWNER_FAX);//FAX:
            log.Debug("Template Head_1=" + sheet.GetRow(2).Cells[2].ToString());
            sheet.GetRow(2).Cells[3].SetCellValue(form.SUPPLIER_ID);//廠商名稱
            log.Debug("Template Head_2=" + sheet.GetRow(3).Cells[2].ToString());
            sheet.GetRow(3).Cells[3].SetCellValue(form.CONTACT_NAME);//聯絡人
            log.Debug("Template Head_3=" + sheet.GetRow(4).Cells[2].ToString());
            sheet.GetRow(4).Cells[3].SetCellValue(form.CONTACT_EMAIL);//電子信箱
            log.Debug("Template Head_4=" + sheet.GetRow(5).Cells[2].ToString());
            sheet.GetRow(5).Cells[3].SetCellValue((form.DUEDATE).ToString());//報價期限
            log.Debug("Template Head_5=" + sheet.GetRow(6).Cells[2].ToString());
            ...
            foreach (TND_PROJECT_FORM_ITEM item in formItems)
            {
                ...
                log.Debug("Inquiry :ITEM DESC=" + item.ITEM_DESC);
        ...
        public void convertInquiry2Project(string fileExcel, string projectid, string iswage)
        {
            ...
            //專案名稱:	P0120
            log.Debug(sheet.GetRow(2).Cells[0].ToString() + "," + sheet.GetRow(2).Cells[1]);
            form.PROJECT_ID = projectid;
            //工資報價單標記
            form.ISWAGE = iswage;
            //廠商名稱:	Supplier
            log.Debug(sheet.GetRow(2).Cells[2].ToString() + "," + sheet.GetRow(2).Cells[3]);
            form.SUPPLIER_ID = sheet.GetRow(2).Cells[3].ToString(); //用供應商名稱暫代供應商編號
                                                                    //採購項目:	 詢價單名稱	
            log.Debug(sheet.GetRow(3).Cells[0].ToString() + "," + sheet.GetRow(3).Cells[1]);
            form.FORM_NAME = sheet.GetRow(3).Cells[1].ToString();
            //聯絡人:	contact
            log.Debug(sheet.GetRow(3).Cells[2].ToString() + "," + sheet.GetRow(3).Cells[3]);
            form.CONTACT_NAME = sheet.GetRow(3).Cells[3].ToString();
            //承辦人:
            log.Debug(sheet.GetRow(4).Cells[0].ToString() + "," + sheet.GetRow(4).Cells[1]);
            form.OWNER_NAME = sheet.GetRow(4).Cells[1].ToString();
            //電子信箱:	contact@email.com
            log.Debug(sheet.GetRow(4).Cells[2].ToString() + "," + sheet.GetRow(4).Cells[3]);
            form.CONTACT_EMAIL = sheet.GetRow(4).Cells[3].ToString();
            //聯絡電話:	08888888				
            log.Debug(sheet.GetRow(5).Cells[0].ToString() + "," + sheet.GetRow(5).Cells[1]);
            form.OWNER_TEL = sheet.GetRow(5).Cells[1].ToString();
            //報價期限:	2017/1/25
            try
            {
                log.Debug(sheet.GetRow(5).Cells[2].ToString() + "," + sheet.GetRow(5).Cells[3].ToString() + "," + sheet.GetRow(5).Cells[3].CellType);
                ...
                log.Debug("form.DUEDATE:" + form.DUEDATE);
            }
            catch (Exception ex)
            {
                log.Error("Datetime format error: " + ex.Message);
                form.DUEDATE = DateTime.Now;
                // throw new Exception("日期格式有錯(YYYY/MM/DD");
            }
            //電子信箱:	admin@Topmep
            log.Debug(sheet.GetRow(6).Cells[0].ToString() + "," + sheet.GetRow(6).Cells[1]);
            form.OWNER_EMAIL = sheet.GetRow(6).Cells[1].ToString();
            //編號: REF - 001
            try
            {
                log.Debug("REF_ID=" + sheet.GetRow(6).Cells[2].ToString() + "," + sheet.GetRow(6).Cells[3]);
                form.FORM_ID = sheet.GetRow(6).Cells[3].ToString().Trim();
            }
            catch (Exception ex)
            {
                form.FORM_ID = "";
                log.Error("Not Reference ID:" + ex.Message);
            }

            //FAX:
            if (sheet.GetRow(7).GetCell(1) != null)
            {
                log.Debug(sheet.GetRow(7).Cells[0].ToString());
            ...
            while (hasMore)
            {
                ...
                log.Info("excel rowid=" + iRowIndex + ",phyCount=" + row.PhysicalNumberOfCells + ",cells count" + row.Cells.Count);
                ...
                {
                    log.Info("Row Index=" + iRowIndex + "column count has wrong" + row.Cells.Count);
                ...
                else
                {
                    try
                    {
                        log.Debug("row id=" + iRowIndex + "Cells Count=" + row.Cells.Count + ",form item vllue:" + row.Cells[0].ToString() + ","
                        ...
                        {
                            log.Warn("Qty format error:" + iRowIndex);
                        }
                        //報價單單價
                        ...
                        {
                            log.Warn("UnitPrice format error:" + iRowIndex);
                        }

                        item.ITEM_REMARK = row.Cells[6].ToString();
                        log.Info("Project ITEM ID=" + row.Cells[row.Cells.Count - 1].ToString());
                    ...
                    {
                        log.Error("Row Data Error:" + iRowIndex);
                        log.Error(ex.GetType() + ":" + ex.StackTrace);
                    }
                }
                iRowIndex++;
            }
    ...
    public class CostAnalysisOutput : ProjectItemFromExcel
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string outputPath = ContextService.strUploadPath;
        string templateFile = ContextService.strTemplatePath + "\\Cost_Analysis_Template_v1.xlsx";
        ...
        public void exportExcel(string projectid)
        {
            ...
            //4取得直接成本
            log.Info("generate direct cost by projectid=" + project.PROJECT_ID);
            getDirectCostFromSouce(service.getDirectCost(project.PROJECT_ID));
            getDirectCostFromMap(service.DirectCost4Project);
            log.Info("generate direct cost End!!");
            //5建立報價標單Sheet
            getFinalQuotation();
            //6建立報價標單Sheet
            log.Info("generate sytem cost by projectid=" + project.PROJECT_ID);
            getSystemCost(service.getSystemCost(project.PROJECT_ID));
            log.Info("generate sytem cost end!!");
            //7.建立預算參考表
            log.Info("generate budget evaluate !!");
            getBudgetEva();
            log.Info("generate budget evaluate end!!");
            //8.令存新檔至專案所屬目錄
            var file = new FileStream(outputPath + "\\" + project.PROJECT_ID + "\\" + project.PROJECT_ID + "_CostAnalysis.xlsx", FileMode.Create);
            log.Info("output file=" + file.Name);
            hssfworkbook.Write(file);
            file.Close();
        }
        //寫入初期成本標單
        private void getInitialQuotation()
        {
            ...
            log.Info("InitialQuotation  Head_1=" + sheet.GetRow(1).Cells[0].ToString() + "," + sheet.GetRow(1).Cells[1].ToString());
            sheet.GetRow(1).Cells[1].SetCellValue(project.PROJECT_ID);//專案編號
            ...
            foreach (PROJECT_ITEM_WITH_WAGE item in projectItems)
            {
                log.Debug("Row Id=" + idxRow + "," + item.ITEM_DESC);
                IRow row = sheet.CreateRow(idxRow);//.GetRow(idxRow);
                //PK(PROJECT_ITEM_ID) 項次 名稱 單位 數量 單價 備註 九宮格 次九宮格 主系統 次系統
                row.CreateCell(0).SetCellValue(item.PROJECT_ITEM_ID);//PK(PROJECT_ITEM_ID)
                ...
                if (null != item.ITEM_UNIT_PRICE && item.ITEM_UNIT_PRICE.ToString().Trim() != "")
                {
                    log.Debug("UNIT PRICE=" + item.ITEM_UNIT_PRICE);
                ...
                {
                    log.Debug("Fomulor=" + "F" + (idxRow + 1) + "*G" + (idxRow + 1));
            ...
            log.Info("InitialQuotation finish!!");
        ...
        private void getFinalQuotation()
        {
            ...
            log.Debug("InitialQuotation  Head_1=" + sheet.GetRow(1).Cells[0].ToString() + "," + sheet.GetRow(1).Cells[1].ToString());
            sheet.GetRow(1).Cells[1].SetCellValue(project.PROJECT_ID);//專案編號
            sheet.GetRow(1).Cells[2].SetCellValue(project.PROJECT_NAME);//專案名稱
            sheet.GetRow(2).Cells[1].SetCellValue(project.LOCATION);//專案名稱
            ...
            foreach (TND_PROJECT_ITEM item in projectItems)
            {
                ...
                log.Debug("set cell style rowid=" + idxRow);
        ...
        private void getDirectCostFromSouce(List<DirectCost> direcCostItems)
        {
            ...
            foreach (DirectCost item in direcCostItems)
            {
                ...
                //分項名稱
                log.Debug("ITEM DESC=" + item.MAINCODE_DESC);
            ...
            //加入小計欄位
            log.Debug("add row for summary:" + idxRow);
        ...
        private void getBudgetEva()
        {
            ...
            {
                IRow row = sheet.CreateRow(idxRow);//.GetRow(idxRow);
                log.Debug("Row Id=" + idxRow + ",ITEM_DESC=" + item.MAINCODE_DESC);
                ...
                log.Debug("getBudget cell style rowid=" + idxRow);
                idxRow++;
            }
            //加入加總欄位
    ...
    public class ProjectItem2Excel : ProjectItemFromExcel
    {
        private static Logger log = NLog.LogManager.GetCurrentClassLogger();
        string outputPath = ContextService.strUploadPath;
        string templateFile = ContextService.strTemplatePath + "\\ProjectItem_Template_v1.xlsx";
        public string errorMsg = "";
        public TND_PROJECT project = null;
        public List<PROJECT_ITEM_WITH_WAGE> projectItems = null;
        CostAnalysisDataService service = new CostAnalysisDataService();
        ...
        protected void getProjectItem()
        {
            ...
            foreach (PROJECT_ITEM_WITH_WAGE item in projectItems)
            {
                ...
                row.CreateCell(1).SetCellValue(item.ITEM_ID);//項次
                row.Cells[1].CellStyle = style;
                log.Debug("ITEM DESC=" + item.ITEM_DESC);
                row.CreateCell(2).SetCellValue(item.ITEM_DESC);//項目說明
                row.Cells[2].CellStyle = style;
                row.CreateCell(3).SetCellValue(item.ITEM_UNIT);// 單位
                row.Cells[3].CellStyle = style;
        ...
        public void ConvertDataForTenderProject(string projectId)
        {
            ...
            if (null == sheet)
            {
                log.Error("檔案內沒有標單品項(Sheet)! filename=" + fileformat);
        ...
        private TND_PROJECT_ITEM convertRow2TndProjectItem4Refresh(int id, IRow row, int excelrow)
        {
            TND_PROJECT_ITEM projectItem = new TND_PROJECT_ITEM();
            projectItem.PROJECT_ID = projId;
            log.Info("excelrow=" + excelrow + ",row count=" + row.Cells.Count + ",phyColumn=" + row.PhysicalNumberOfCells);
            string strVal = "";
            int i = 0;
            ...
            log.Debug("Excel Row=" + excelrow + ",value=" + strVal);
            //至少需要兩個Column
            ...
            if (row.Cells[6].ToString().Trim() != "")//單價  6
            {
                try
                {
                    ...
                    log.Info("excelrow=" + excelrow + ",value=" + row.Cells[4].ToString());
            ...
            else
            {
                ...
                log.Debug("excel row:" + excelrow + ",type_code1=" + projectItem.TYPE_CODE_1 + ",type_code2=" + projectItem.TYPE_CODE_2 + ",system_main=" + projectItem.SYSTEM_MAIN + ",system_sub=" + projectItem.SYSTEM_SUB);
            }
            log.Info("TndprojectItem=" + projectItem.ToString());
            return projectItem;
        }
