...
namespace Topmep.Service
{
    public class PlanService : ContextService
    {
        ...
        public int addBudget(List<PLAN_BUDGET> lstItem)
        {
            //1.新增預算資料
            int i = 0;
            log.Info("add budget = " + lstItem.Count);
            //2.將預算資料寫入 
            using (var context = new TopmepEntities())
            {
        ...
        public int updateBudget(string projectid, List<PLAN_BUDGET> lstItem)
        {
            //1.修改預算資料
            int i = 0;
            log.Info("update budget = " + lstItem.Count);
            //2.將預算資料寫入 
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_BUDGET item in lstItem)
                {
                    ...
                    log.Debug("plan budget id=" + item.PLAN_BUDGET_ID);
                    ...
                    else
                    {
                        ...
                        log.Info(sql + " ;" + item.PROJECT_ID + item.TYPE_CODE_1 + item.TYPE_CODE_2 + item.SYSTEM_MAIN + item.SYSTEM_SUB);
                    ...
                    log.Debug("find exist item=" + existItem.PLAN_BUDGET_ID);
            ...
            log.Info("update budget count =" + i);
            return i;
        }
        public int updateBudgetToPlanItem(string id)
        {
            int i = 0;
            log.Info("update budget ratio to plan items by id :" + id);
        ...
        public int updateItemBudget(string projectid, List<PLAN_ITEM> lstItem)
        {
            ...
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_ITEM item in lstItem)
                {
                    ...
                    log.Debug("plan item id=" + item.PLAN_ITEM_ID);
                    ...
                    else
                    {
                        ...
                        log.Info(sql + " ;" + item.PROJECT_ID + item.PLAN_ITEM_ID);
                    ...
                    log.Debug("find exist item=" + existItem.PLAN_ITEM_ID);
                    existItem.BUDGET_RATIO = item.BUDGET_RATIO;
        ...
        public int refreshBudget(List<PLAN_BUDGET> items)
        {
            ...
            int i = 0;
            log.Info("refreshBudgetItem = " + items.Count);
            //2.將Excel 資料寫入 
            using (var context = new TopmepEntities())
            {
            ...
            log.Info("add PLAN_BUDGET count =" + i);
            return i;
        }
        public int delBudgetByProject(string projectid)
        {
            log.Info("remove all budget by project ID=" + projectid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete all PLAN_BUDGET by proejct id=" + projectid);
            ...
            log.Debug("delete PLAN_BUDGET count=" + i);
            return i;
        }
        #endregion
        ...
        public PlanRevenue getPlanRevenueById(string prjid)
        {
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("sql=" + sql + ",project_id=" + prjid);
        ...
        public int delOwnerContractByProject(string projectId)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("delete owner contract by proejct id=" + projectId);
            ...
            log.Debug("delete owner contract count =" + i);
            return i;
        }
        ...
        public int AddOwnerContractProcess(PLAN_CONTRACT_PROCESS contract)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("add owner contract process fail:" + e.ToString());
        ...
        public int addContractId4Owner(string projectid)
        {
            ...
            log.Info("copy contract id from owner into plan payment terms, project id =" + projectid);
            using (var context = new TopmepEntities())
            {
        ...
        public List<RevenueFromOwner> getOwnerContractFileByPrjId(string projectid)
        {
            ...
            log.Info(" get owner's contract file by project id =" + projectid);
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get owner's contract file sql=" + sql);
                lstItem = context.Database.SqlQuery<RevenueFromOwner>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get owner's contract file record count=" + lstItem.Count);
            return lstItem;
        }
    }
    //採發階段
    ...
    public class BudgetDataService : CostAnalysisDataService
    {
        ...
        public List<DirectCost> getBudget(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("sql = " + sql + ",project_id=" + projectid);
                var parameters = new List<SqlParameter>();
                parameters.Add(new SqlParameter("projectid", projectid));
                lstBudget = context.Database.SqlQuery<DirectCost>(sql, parameters.ToArray()).ToList();
                log.Info("Get Budget Info Record Count=" + lstBudget.Count);
            }
            return lstBudget;
        }
        //取得投標標單總直接成本
        ...
        {
            log.Info("search plan item by 九宮格 =" + typeCode1 + "search plan item by 次九宮格 =" + typeCode2 + "search plan item by 主系統 =" + systemMain + "search plan item by 次系統 =" + systemSub + "search plan item by 採購名稱 =" + formName);
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get plan item sql=" + sql);
...
";
        ...
        public List<PlanItem4Map> getPlanItem(string checkEx, string projectid, string typeCode1, string typeCode2, string systemMain, string systemSub, string formName, string supplier, string delFlg, string inContractFlg)
        {
            ...
            log.Info("search plan item by 九宮格 =" + typeCode1 + "search plan item by 次九宮格 =" + typeCode2 + "search plan item by 主系統 =" + systemMain + "search plan item by 次系統 =" + systemSub + "search plan item by 採購項目 =" + formName + "search plan item by 材料供應商 =" + supplier);
            List<Topmep.Models.PlanItem4Map> lstItem = new List<PlanItem4Map>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            log.Info("get plan item count=" + lstItem.Count);
            return lstItem;
        }
        #endregion
        ...
        {
            log.Debug("get plan item by id=" + itemid);
        ...
        public int updatePlanItem(PLAN_ITEM item)
        {
            ...
            {
                log.Debug("add new plan item in porjectid=" + item.PROJECT_ID);
                item = getNewPlanItemID(item);
            }
            log.Debug("plan item key=" + item.PLAN_ITEM_ID);
            using (var context = new TopmepEntities())
            {
                try
                {
                ...
                {
                    log.Error("updatePlanItem  fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
            }
        ...
        private PLAN_ITEM getNewPlanItemID(PLAN_ITEM item)
        {
            ...
            log.Debug("new plan item id=" + longMaxItem + ",ExcelRowID=" + longMaxExcel);
        ...
        public int changePlanItem(string itemid, string delFlag)
        {
            ...
            using (var db = new TopmepEntities())
            {
                ...
                log.Info("Update PLAN_ITEM FLAG=" + sql + ",itemid=" + itemid + ",delFlag=" + delFlag);
        ...
        public int createPlanEmptyForm(string projectid, SYS_USER loginUser)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                foreach (TYPE_CODE_INDEX idx in lstType)
                {
                    ...
                    log.Debug("get plan item count=" + lstPlanItem.Count + ", by typecode1=" + idx.TYPE_CODE_1 + ",typeCode2=" + idx.TYPE_CODE_2);
        ...
        {
            //1.建立詢價單價單樣本
            log.Info("create new plan form ");
            string sno_key = "PP";
            SerialKeyService snoservice = new SerialKeyService();
            form.INQUIRY_FORM_ID = snoservice.getSerialKey(sno_key);
            log.Info("new plan form =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.PLAN_SUP_INQUIRY.Add(form);
                int i = context.SaveChanges();
                log.Debug("Add form=" + i);
                log.Info("plan form id = " + form.INQUIRY_FORM_ID);
                //if (i > 0) { status = true; };
                List<Topmep.Models.PLAN_SUP_INQUIRY_ITEM> lstItem = new List<PLAN_SUP_INQUIRY_ITEM>();
                string ItemId = "";
        ...
        {
            log.Info("get purchase template by projectid=" + projectid);
        ...
        {
            log.Info("get purchase template by projectid=" + project.PROJECT_ID);
...
";
                ...
                log.Debug("sql=" + sql + ",projectId=" + project.PROJECT_ID);
        ...
        "AND a.PROJECT_ID =@projectid ORDER BY a.INQUIRY_FORM_ID DESC, a.FORM_NAME ;";
            log.Info("sql4SupInqueryForm=" + sql);
            var parameters = new List<SqlParameter>();
            //設定專案編號資料
            parameters.Add(new SqlParameter("projectid", projectid));
            //設定詢價單是否有效
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("get plan supplier form function count:" + lst.Count);
            }
            return lst;
        }
        /// <summary>
        ...
        {
            log.Info("delete contract form by proejct id=" + projectId + ",formId=" + formId);
            ...
            log.Debug("Cance Form:" + sql + sqlUpdatePlanItem);
            using (var context = new TopmepEntities())
            {
        ...
        {
            log.Info("get purchase template out of contract by projectid=" + projectid);
            List<PURCHASE_ORDER> lst = new List<PURCHASE_ORDER>();
            using (var context = new TopmepEntities())
            {
                //取得詢價單樣本資訊
        ...
        public int delAllContractByProject(string projectId)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("delete all contract by proejct id=" + projectId);
                i = context.Database.ExecuteSqlCommand("DELETE FROM PLAN_CONTRACT_PROCESS WHERE PROJECT_ID=@projectid AND CONTRACT_ID <> @projectid ", new SqlParameter("@projectid", projectId));
            }
            log.Debug("delete contract count=" + i);
            return i;
        }
        //新增合約製作狀態
        public int AddContractProcess(string projectId, List<PLAN_CONTRACT_PROCESS> lstItem)
        {
            log.Info("Add plan contract process by project id =" + projectId);
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    foreach (PLAN_CONTRACT_PROCESS item in lstItem)
                    {
                        ...
                        log.Debug("item contract id=" + item.CONTRACT_ID);
                    ...
                    log.Debug("Add plan contract process item =" + j);
                }
                catch (Exception e)
                {
                    log.Error("update new plan supplier form id fail:" + e.Message);
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
            }
        ...
        public int addFormName(List<PLAN_SUP_INQUIRY> lstItem)
        {
            ...
            using (var context = new TopmepEntities())
            {
                try
                {
                    log.Info(" No. of plan form to refresh  = " + lstItem.Count);
                    //2.將plan form資料寫入 
                    foreach (PLAN_SUP_INQUIRY item in lstItem)
                    {
                        PLAN_SUP_INQUIRY existItem = null;
                        log.Debug("plan form id=" + item.INQUIRY_FORM_ID);
                        ...
                        else
                        {
                            ...
                            log.Info(sql + " ;" + item.INQUIRY_FORM_ID);
                        ...
                        log.Debug("find exist item=" + existItem.PROJECT_ID + " ;" + existItem.INQUIRY_FORM_ID);
                        existItem.FORM_NAME = item.FORM_NAME;
                        context.PLAN_SUP_INQUIRY.AddOrUpdate(existItem);
                    }
                    i = context.SaveChanges();
                    log.Debug("No. of update plan form =" + i);
                    return i;

                }
                catch (Exception e)
                {
                    log.Error("update  plan  form  fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
            }
        ...
        public string addSupplierForm(PLAN_SUP_INQUIRY sf, string[] lstItemId)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("add new plan supplier form id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        {
            log.Info("Update plan supplier inquiry form id =" + formid);
            form = sf;
            int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    i = context.SaveChanges();
                    log.Debug("Update plan supplier inquiry form =" + i);
                    ...
                    foreach (PLAN_SUP_INQUIRY_ITEM item in lstItem)
                    {
                        ...
                        log.Debug("form item id=" + item.INQUIRY_ITEM_ID);
                        ...
                        else
                        {
                            ...
                            log.Info(sql + " ;" + formid + ",plan_item_id=" + item.PLAN_ITEM_ID);
                    ...
                    log.Debug("Update plan supplier inquiry form item =" + j);
                    return j;
                }
                catch (Exception e)
                {
        ...
        protected void deletePlanSupInquiryItem(string formid)
        {
            ...
            log.Info("sql=" + sqlDel + ",formId=" + formid);
            try
            {
                using (var context = new TopmepEntities())
                {
        ...
        public int createPlanFormFromSupplier(PLAN_SUP_INQUIRY form, List<PLAN_SUP_INQUIRY_ITEM> items)
        {
            ...
            SerialKeyService snoservice = new SerialKeyService();
            form.INQUIRY_FORM_ID = snoservice.getSerialKey(sno_key);
            log.Info("Plan form from supplier =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.PLAN_SUP_INQUIRY.Add(form);
        ...
        public List<string> getSupplierForContract(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("Get Supplier For Contract Count=" + lst.Count);
            }
            return lst;
        }
        ...
        {
            log.Info("get boolean of formid in the plan item by formid=" + formid);
...
WHERE PROJECT_ID=@projectid  AND ('('+ TYPE_CODE_1 + ','+TYPE_CODE_2 +')' + BUDGET_NAME)=VAL.FORM_NAME) BUDGET,
                ...
                //context.Database.Log = (log) => log.Debug(log);
            }
            return lst;
        }
        ...
        public List<purchasesummary> getPurchaseForm4Offer(string projectid, string formname, string iswage)
        {
            ...
            log.Info("search purchase form by 採購項目 =" + formname);
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get purchase form sql=" + sql);
                lstForm = context.Database.SqlQuery<purchasesummary>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get purchase form count=" + lstForm.Count);
            return lstForm;
        }
        ...
        fitem.ITEM_UNIT_PRICE  
          ...
          where pitem.PROJECT_ID = @projectid ";
                ...
                {
                    log.Debug("Supplier=" + it.SUPPLIER_NAME + "," + it.INQUIRY_FORM_ID + "," + it.FORM_NAME);
                    if (dimString == "")
                    {
        ...
        {
            int i = 0;
            log.Info("Remove plan supplier form from Quote by form id" + formid);
            string sql = "UPDATE  PLAN_SUP_INQUIRY SET STATUS = '註銷', MODIFY_DATE = getdate() WHERE INQUIRY_FORM_ID=@formid ";
            log.Debug("remove form from Quote sql:" + sql);
            db = new TopmepEntities();
            var parameters = new List<SqlParameter>();
            parameters.Add(new SqlParameter("formid", formid));
            db.Database.ExecuteSqlCommand(sql, parameters.ToArray());
        ...
        {
            int i = 0;
            log.Info("Filter plan supplier form from Quote by form id" + formid);
        ...
        {
            int i = 0;
            log.Info("Update Cost:plan item id=" + planItemid + ",price=" + price);
            db = new TopmepEntities();
        ...
        public int addContractId(string projectid)
        {
            ...
            log.Info("copy contract id into plan payment terms, project id =" + projectid);
            using (var context = new TopmepEntities())
            {
                List<Topmep.Models.PLAN_PAYMENT_TERMS> lstItem = new List<PLAN_PAYMENT_TERMS>();
                string sql = "INSERT INTO PLAN_PAYMENT_TERMS (CONTRACT_ID, PROJECT_ID, TYPE) " +
        ...
        {
            int i = 0;
            log.Info("copy contract id from wage into plan payment terms, project id =" + projectid);
            using (var context = new TopmepEntities())
            {
                List<Topmep.Models.PLAN_PAYMENT_TERMS> lstItem = new List<PLAN_PAYMENT_TERMS>();
                string sql = "INSERT INTO PLAN_PAYMENT_TERMS (CONTRACT_ID, PROJECT_ID, TYPE) " +
        ...
        public List<PLAN_ITEM> getContractItemsByContractName(string contractid)
        {
            ...
            log.Info("get plan supplier contract items count:" + lst.Count);
            return lst;
        }

        public PaymentTermsFunction getPaymentTerm(string contractid, string estid)
        {
            log.Debug("get payment terms by contractid=" + contractid + ",estid=" + estid);
        ...
        public List<PlanItem4Map> getPendingItems(string projectid)
        {
            ...
            log.Info("get plan pending items count:" + lst.Count);
            return lst;
        }
        ...
        public int updatePaymentTerms(PLAN_PAYMENT_TERMS item)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("updatePaymentTerms fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        {
            int i = 0;
            log.Info("Update plan sup inquiry form status formid=" + formid + ",status=" + status);
            db = new TopmepEntities();
            ...
            i = db.SaveChanges();
            db = null;
            log.Debug("Update plan sup inquiry form status  :" + i);
            return i;
        }
        //更新得標標單品項之採購前置天數
        public int updateLeadTime(string projectid, List<PLAN_ITEM> lstItem)
        {
            //1.新增前置天數資料
            int i = 0;
            log.Info("update lead time = " + lstItem.Count);
            //2.將預算資料寫入 
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_ITEM item in lstItem)
            ...
            log.Info("update lead time count =" + i);
            return i;
        }
...
LEFT JOIN (SELECT pri.PLAN_ITEM_ID, SUM(pri.ORDER_QTY) AS CUMULATIVE_QTY 
                ...
                log.Info("Get task material Info Record Count=" + lstItem.Count);
            }
            return lstItem;
        }
        ...
        public string newPR(string projectid, PLAN_PURCHASE_REQUISITION form, string[] lstItemId)
        {
            ...
            log.Info("new purchase requisition =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.PLAN_PURCHASE_REQUISITION.Add(form);
                int i = context.SaveChanges();
                log.Debug("Add Purchase Requisition=" + i);
                log.Info("plan purchase requisition id = " + form.PR_ID);
                //if (i > 0) { status = true; };
                List<Topmep.Models.PLAN_PURCHASE_REQUISITION_ITEM> lstItem = new List<PLAN_PURCHASE_REQUISITION_ITEM>();
                string ItemId = "";
                for (i = 0; i < lstItemId.Count(); i++)
                {
                    if (i < lstItemId.Count() - 1)
        ...
        {
            log.Info("Update plan purchase requisition id =" + formid);
            int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    context.Entry(form).State = EntityState.Modified;
                    i = context.SaveChanges();
                    log.Debug("Update plan purchase requisition =" + i);
                    log.Info("purchase requisition item = " + lstItem.Count);
                    //2.將item資料寫入 
                    ...
                    j = context.SaveChanges();
                    log.Debug("Update purchase requisition item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new purchase requisition id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        public List<PRFunction> getPRFunction(string type, string projectid, string status, string yymm, string prid, string keyword)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get purchase requisition sql=" + sql);
                lstForm = context.Database.SqlQuery<PRFunction>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get purchase requisition count=" + lstForm.Count);
            return lstForm;
        }
        //取得申購單資料
        public List<PRFunction> getPRByPrjId(string projectid, string date, string keyname, string prid, string status)
        {
            log.Info("search purchase requisition by 申購日期 =" + date + ", 申購單編號 =" + prid + ", 關鍵字 =" + keyname + ", 申購單狀態 =" + status);
...
";
                log.Debug("get purchase requisition item :" + sql4Po);
                ...
                log.Debug("get purchase requisition item :" + sql4Po + ",prid=" + prid);
                DOItem = context.Database.SqlQuery<PurchaseRequisition>(sql4Deliver, new SqlParameter("prid", prid)).ToList();

                log.Debug("get delivery item count:" + DOItem.Count);
            }
        }
        ...
        public int addPRItem(PLAN_PURCHASE_REQUISITION_ITEM item)
        {
            ...
            using (var context = new TopmepEntities())
            {
                try
                {
                    log.Debug("INSERT ITEM TO PLAN_PURCHASE_REQUISITION:" + item.PR_ID);
        ...
        {
            log.Info("Update purchase requisition id =" + formid);
            table = pr;
            int i = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    i = context.SaveChanges();
                    log.Debug("Update purchase requisition =" + i);
                    log.Info("purchase requisition item = " + lstItem.Count);
                    //2.將item資料寫入 
                    foreach (PLAN_PURCHASE_REQUISITION_ITEM item in lstItem)
                    {
                        PLAN_PURCHASE_REQUISITION_ITEM existItem = null;
                        log.Debug("purchase requisition item id=" + item.PR_ITEM_ID);
                    ...
                    log.Debug("Update purchase requisition item =" + i);
                }
                catch (Exception e)
                {
                    log.Error("update new purchase requisition id fail:" + e.ToString());
        ...
        public List<PurchaseOrderFunction> getPRBySupplier(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("Get Purchase Requisition By Suplier Record Count =" + lstPO.Count);
            }
            return lstPO;
        }
        //取得申購單項目by供應商
        ...
        {
            //取得各供應商採購內容
            log.Info("get purchase requisition item by supplier ");
            List<PurchaseRequisition> lstItem = new List<PurchaseRequisition>();
            using (var context = new TopmepEntities())
            {
                ...
                lstItem = context.Database.SqlQuery<PurchaseRequisition>(sql, parameters.ToArray()).ToList();
                log.Info("Get purchase requisition item by supplier Record Count=" + lstItem.Count);
            }
            return lstItem;
        }
        ...
        {
            int i = 0;
            log.Info("Update PR Status and Message, it's formid=" + formid + ", messaage =" + message);
            db = new TopmepEntities();
        ...
        {
            int i = 0;
            log.Info("reject PR form by prid" + prid);
            using (var context = new TopmepEntities())
            {
        ...
        protected void UpdateStatus(string prid, string parent_id, string status, TopmepEntities context)
        {
            ...
            log.Debug("sql=" + sql + ",status=" + status + ",prid=" + prid + ",parend_pr_id" + parent_id);
        ...
        public string newPO(string projectid, PLAN_PURCHASE_REQUISITION form, string[] lstItemId, string parentid)
        {
            ...
            SerialKeyService snoservice = new SerialKeyService();
            form.PR_ID = snoservice.getSerialKey(sno_key);
            log.Info("new purchase order =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.PLAN_PURCHASE_REQUISITION.Add(form);
                int i = context.SaveChanges();
                log.Debug("Add Purchase Order=" + i);
                log.Info("plan purchase Order id = " + form.PR_ID);
                //if (i > 0) { status = true; };
                List<Topmep.Models.PLAN_PURCHASE_REQUISITION_ITEM> lstItem = new List<PLAN_PURCHASE_REQUISITION_ITEM>();
                string ItemId = "";
                for (i = 0; i < lstItemId.Count(); i++)
        ...
        {
            log.Info("Update plan purchase order id =" + formid);
            //int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    context.Entry(form).State = EntityState.Modified;
                    i = context.SaveChanges();
                    log.Debug("Update plan purchase order =" + i);
                    log.Info("purchase order item = " + lstItem.Count);
                    //2.將item資料寫入 
                    foreach (PLAN_PURCHASE_REQUISITION_ITEM item in lstItem)
                    {
                        PLAN_PURCHASE_REQUISITION_ITEM existItem = null;
                    ...
                    log.Debug("Update purchase order item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new purchase order id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        public List<PRFunction> getPOByPrjId(string projectid, string date, string supplier, string prid, string parentPrid, string keyname)
        {
            ...
            log.Info("search purchase order by 採購日期 =" + date + ", 採購單編號 =" + prid + ", 供應商名稱 =" + supplier + ", 申購單編號 =" + parentPrid + ", 關鍵字 =" + keyname);
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get purchase order sql=" + sql);
                lstForm = context.Database.SqlQuery<PRFunction>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get purchase order count=" + lstForm.Count);
            return lstForm;
        }
        ...
        {
            log.Info("Update purchase order id =" + formid);
            table = pr;
            int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    context.Entry(table).State = EntityState.Modified;
                    i = context.SaveChanges();
                    log.Debug("Update purchase order =" + i);
                    log.Info("purchase order item = " + lstItem.Count);
                    //2.將item資料寫入 
                    foreach (PLAN_PURCHASE_REQUISITION_ITEM item in lstItem)
                    {
                        PLAN_PURCHASE_REQUISITION_ITEM existItem = null;
                        log.Debug("purchase order item id=" + item.PR_ITEM_ID);
                        if (item.PR_ITEM_ID != 0)
                        {
                            existItem = context.PLAN_PURCHASE_REQUISITION_ITEM.Find(item.PR_ITEM_ID);
                        }
        ...
        {
            int i = 0;
            log.Info("Update PO memo, it's formid=" + formid + ", memo =" + memo);
            db = new TopmepEntities();
        ...
        public string newRP(string projectid, PLAN_PURCHASE_REQUISITION form, string[] lstItemId, string parentid)
        {
            ...
            SerialKeyService snoservice = new SerialKeyService();
            form.PR_ID = snoservice.getSerialKey(sno_key);
            log.Info("new purchase receipt =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.PLAN_PURCHASE_REQUISITION.Add(form);
                int i = context.SaveChanges();
                log.Debug("Add Purchase receipt=" + i);
                log.Info("plan purchase receipt id = " + form.PR_ID);
                //if (i > 0) { status = true; };
                List<Topmep.Models.PLAN_PURCHASE_REQUISITION_ITEM> lstItem = new List<PLAN_PURCHASE_REQUISITION_ITEM>();
                string ItemId = "";
                for (i = 0; i < lstItemId.Count(); i++)
        ...
        {
            log.Info("Update plan purchase receipt id =" + formid);
            int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    context.Entry(form).State = EntityState.Modified;
                    i = context.SaveChanges();
                    log.Debug("Update plan purchase receipt =" + i);
                    log.Info("purchase receipt item = " + lstItem.Count);
                    //2.將item資料寫入 
                    foreach (PLAN_PURCHASE_REQUISITION_ITEM item in lstItem)
                    {
                        PLAN_PURCHASE_REQUISITION_ITEM existItem = null;
                    ...
                    j = context.SaveChanges();
                    log.Debug("Update purchase reeipt item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new purchase receipt id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        //{
        //    log.Info("Update purchase receipt id =" + formid);
        //    table = pr;
        //    int i = 0;
        //    int j = 0;
        //    using (var context = new TopmepEntities())
        ...
        //            context.Entry(table).State = EntityState.Modified;
        //            i = context.SaveChanges();
        //            log.Debug("Update purchase receipt =" + i);
        //            log.Info("purchase receipt item = " + lstItem.Count);
        //            //2.將item資料寫入 
        //            foreach (PLAN_PURCHASE_REQUISITION_ITEM item in lstItem)
        //            {
        //                PLAN_PURCHASE_REQUISITION_ITEM existItem = null;
        //                log.Debug("purchase receipt item id=" + item.PR_ITEM_ID);
        //                if (item.PR_ITEM_ID != 0)
        //                {
        //                    existItem = context.PLAN_PURCHASE_REQUISITION_ITEM.Find(item.PR_ITEM_ID);
        //                }
        ...
        //            j = context.SaveChanges();
        //            log.Debug("Update purchase receipt item =" + j);
        //            return j;
        //        }
        //        catch (Exception e)
        //        {
        ...
        public List<PRFunction> getRPByPrId(string prid)
        {
            ...
            log.Info("search purchase receipt by 採購單編號 =" + prid);
            List<PRFunction> lstForm = new List<PRFunction>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get purchase receipt sql=" + sql);
                lstForm = context.Database.SqlQuery<PRFunction>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get purchase receipt count=" + lstForm.Count);
            return lstForm;
        }
        ...
        public List<PRFunction> getRP4Delivery(string prjid, string keyword)
        {
            ...
            log.Info("search receipt for delivery by 專案編號 =" + prjid + ", 關鍵字名稱 =" + keyword);
            List<PRFunction> lstForm = new List<PRFunction>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get receipt for delivery sql=" + sql);
                lstForm = context.Database.SqlQuery<PRFunction>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get receipt for delivery count=" + lstForm.Count);
            return lstForm;
        }
        //取得物料庫存數量
        public List<PurchaseRequisition> getInventoryByPrjId(string prjid, string itemName, string typeMain, string typeSub, string systemMain, string systemSub, string remark)
        {
            ...
            log.Info("search inventory by 專案編號 =" + prjid + ", 物料名稱 =" + itemName + ", 九宮格 =" + typeMain + ", 次九宮格 =" + typeSub + ", 主系統 =" + systemMain + ", 次系統 =" + systemSub + ", 備註 =" + remark);
            List<PurchaseRequisition> lstItem = new List<PurchaseRequisition>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get inventory sql=" + sql);
                lstItem = context.Database.SqlQuery<PurchaseRequisition>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get inventory count=" + lstItem.Count);
            return lstItem;
        }
        ...
        {
            //1.新增領料品項
            log.Info("create new delivery item ");
            string sno_key = "DO";
            SerialKeyService snoservice = new SerialKeyService();
            form.PR_ID = snoservice.getSerialKey(sno_key);
            log.Info("new delivery form =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.PLAN_PURCHASE_REQUISITION.Add(form);
                int i = context.SaveChanges();
                log.Debug("Add Delivery Form=" + i);
                log.Info("plan delivery form id = " + form.PR_ID);
                //if (i > 0) { status = true; };
                List<Topmep.Models.PLAN_ITEM_DELIVERY> lstItem = new List<PLAN_ITEM_DELIVERY>();
                string ItemId = "";
                for (i = 0; i < lstItemId.Count(); i++)
                {
                    if (i < lstItemId.Count() - 1)
        ...
        {
            log.Info("Update delivery items, it's delivery order id =" + deliveryorderid);
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    foreach (PLAN_ITEM_DELIVERY item in lstItem)
                    {
                        ...
                        log.Info(sql + " ;" + deliveryorderid + ",plan_item_id=" + item.PLAN_ITEM_ID + ",remark=" + item.REMARK);
                    ...
                    j = context.SaveChanges();
                    log.Debug("Update delivery item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new delivery id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        public List<PurchaseRequisition> getDeliveryByItemId(string itemid)
        {
            ...
            log.Info(" get receipt record and delivery record by 物料編號 =" + itemid);
            List<PurchaseRequisition> lstItem = new List<PurchaseRequisition>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get receipt and delivery sql=" + sql);
                lstItem = context.Database.SqlQuery<PurchaseRequisition>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get receipt record and delivery record count=" + lstItem.Count);
            return lstItem;
        }
        ...
        {
            //1.建立領料單
            log.Info("create new delivery form ");
            string sno_key = "DF";
            SerialKeyService snoservice = new SerialKeyService();
            form.PR_ID = snoservice.getSerialKey(sno_key);
            log.Info("new delivery form =" + form.ToString());
        ...
        public List<PRFunction> getDOByPrjId(string projectid, string recipient, string prid, string caution)
        {
            ...
            log.Info("search delivery form by 領料說明 =" + caution + ", 領料單編號 =" + prid + ", 領料人所屬單位 =" + recipient);
            List<PRFunction> lstForm = new List<PRFunction>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get delivery form sql=" + sql);
                lstForm = context.Database.SqlQuery<PRFunction>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get delivery form count=" + lstForm.Count);
            return lstForm;
        }
        ...
        public PurchaseRequisition getInventoryByItemId(string itemid)
        {
            ...
            log.Info("search item inventory by planitemid  =" + itemid);
        ...
        public List<PurchaseRequisition> getPlanItemByNeedDate(string prid, string prjid, int status)
        {
            ...
            log.Info(" get materials ready to receive in 3 days by 申購單編號 =" + prid + ",and project id =" + prjid + ",and status = " + status);
            List<PurchaseRequisition> lstItem = new List<PurchaseRequisition>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get materials ready to receive in 3 days sql=" + sql);
                lstItem = context.Database.SqlQuery<PurchaseRequisition>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get material's item count ready to receive in 3 days =" + lstItem.Count);
            return lstItem;
        }
        ...
        public List<PurchaseRequisition> getItemNeedReceivedByPrId(string prid, string prjid, int status)
        {
            ...
            log.Info(" get materials need received by 申購單編號 =" + prid + ",and project id =" + prjid + ",and status = " + status);
            List<PurchaseRequisition> lstItem = new List<PurchaseRequisition>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get materials need received sql=" + sql);
                lstItem = context.Database.SqlQuery<PurchaseRequisition>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get material's item count ready to receive in 3 days =" + lstItem.Count);
            return lstItem;
        ...
        {
            //1.建立估驗單
            log.Info("create new estimation form ");
            using (var context = new TopmepEntities())
            {
                ...
                //if (i > 0) { status = true; };
                /*
                List<Topmep.Models.PLAN_ESTIMATION_ITEM> lstItem = new List<PLAN_ESTIMATION_ITEM>();
                string ItemId = "";
                for (i = 0; i < lstItemId.Count(); i++)
                {
                    if (i < lstItemId.Count() - 1)
        ...
        {
            log.Info("Update plan estimation form id =" + formid);
            int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    context.Entry(form).State = EntityState.Modified;
                    i = context.SaveChanges();
                    log.Debug("Update plan estimation form =" + i);
                    log.Info("purchase estimation item = " + lstItem.Count);
                    //2.將item資料寫入 
                    ...
                    j = context.SaveChanges();
                    log.Debug("Update plan estimation item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new estimation form id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        {
            log.Info("search estimation form by 估驗單編號 =" + estid + ", 合約名稱 =" + contractid + ", 估驗單狀態 =" + status);
            ...
            if (20 == status)
            {
                ...
                using (var context = new TopmepEntities())
                {
                    log.Debug("get estimation form sql=" + sql);
                    lstForm = context.Database.SqlQuery<ESTFunction>(sql, parameters.ToArray()).ToList();
                }
                log.Info("get estimation form count=" + lstForm.Count);
            }
            else
            {
                string sql = "SELECT CONVERT(char(10), A.CREATE_DATE, 111) AS CREATE_DATE, A.EST_FORM_ID, A.STATUS, A.CONTRACT_NAME, A.SUPPLIER_NAME, ROW_NUMBER() OVER(ORDER BY A.EST_FORM_ID) AS NO " +
        ...
        {
            log.Info("get form : formid=" + estid);
            formEST = null;
            using (var context = new TopmepEntities())
            {
                //取得估驗單檔頭資訊
                ...
                log.Debug("get estimation form item count:" + ESTItem.Count);
            }
        }
        ...
        public int addOtherPayment(List<PLAN_OTHER_PAYMENT> lstItem)
        {
            //1.新增其他扣款資料
            int i = 0;
            log.Info("add other payment = " + lstItem.Count);
            //2.將扣款資料寫入 
            using (var context = new TopmepEntities())
            {
...
";
            ...
            log.Info("add other payment count =" + i);
            return i;
        }
        //取得估驗單其他扣款明細資料
        public List<PLAN_OTHER_PAYMENT> getOtherPayById(string id)
        {
            ...
            log.Info("get other payment by EST id =" + id);
        ...
        {
            int status = -10;
            log.Info("get EST status by EST id =" + id);
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        {
            log.Info("remove all other payment detail by EST FORM ID=" + estid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete these other payment record by est form id=" + estid);
            ...
            log.Debug("delete PLAN OTHER PAYMENT count=" + i);
            return i;
        }
        ...
        {
            log.Info("Update estiomation items, it's est form id =" + formid);
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    j = context.SaveChanges();
                    log.Debug("Update estimation item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new est form id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        public AdvancePaymentFunction getAdvancePayById(string id, string contractid)
        {
            ...
            log.Info("get advance payment by EST id + contractid  =" + id);
            using (var context = new TopmepEntities())
            {
        ...
        public int addAdvancePayment(List<PLAN_OTHER_PAYMENT> lstItem)
        {
            ...
            log.Info("add advance payment = " + lstItem.Count);
            //2.將預付款資料寫入 
            using (var context = new TopmepEntities())
            {
                var parameters = new List<SqlParameter>();
            ...
            log.Info("add advance payment count =" + i);
            return i;
        }
        ...
        public List<PLAN_OTHER_PAYMENT> getAdvancePayByESTId(string id)
        {
            ...
            log.Info("get advance payment by EST id =" + id);
            List<PLAN_OTHER_PAYMENT> lstItem = new List<PLAN_OTHER_PAYMENT>();
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        public int updateAdvancePayment(string estid, List<PLAN_OTHER_PAYMENT> lstItem)
        {
            //1.修改預付款資料
            int i = 0;
            log.Info("update advance payment = " + lstItem.Count);
            //2.將預付款資料寫入 
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_OTHER_PAYMENT item in lstItem)
                {
                    ...
                    log.Info(sql + " ;" + item.EST_FORM_ID + item.TYPE);
                    ...
                    log.Debug("find exist item=" + existItem.TYPE);
                    existItem.AMOUNT = item.AMOUNT;
                    context.PLAN_OTHER_PAYMENT.AddOrUpdate(existItem);
                }
                i = context.SaveChanges();
            }
            log.Info("update advance payment count =" + i);
            return i;
        }
        ...
        {
            log.Info("get details payment by  id  =" + formid);
            using (var context = new TopmepEntities())
            {
                string sql = @"
                    SELECT D.*,
        ...
        {
            int i = 0;
            log.Info("update retention payment of EST form by id" + formid);
        ...
        {
            int i = 0;
            log.Info("update sub amount and amount need to pay from EST form by id" + formid);
        ...
        {
            log.Info("remove EST form detail by EST FORM ID =" + estid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete EST form record by est form id =" + estid);
            ...
            log.Debug("delete PLAN ESTIMATION FORM count=" + i);
            return i;
        }

        public int delESTItemsByESTId(string estid)
        {
            log.Info("remove EST items detail by EST FORM ID  =" + estid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete EST items record by est form id =" + estid);
            ...
            log.Debug("delete PLAN ESTIMATION ITEM count=" + i);
            return i;
        }
        //更新估驗單狀態為草稿
        ...
        {
            int i = 0;
            log.Info("update the status of EST form by estid" + estid);
        ...
        {
            int i = 0;
            log.Info("update EST form by estid" + estid);
        ...
        {
            int i = 0;
            log.Info("reject EST form by estid" + estid);
        ...
        {
            int i = 0;
            log.Info("Approve EST form by estid" + estid);
        ...
        */
        ...
        public List<PLAN_INVOICE> getInvoiceById(string id)
        {
            ...
            log.Info("get invoice by EST id =" + id);
        ...
        public List<PLAN_INVOICE> getInvoiceByContractId(string formid, string contractid)
        {
            ...
            log.Info("get invoice by contract id =" + contractid);
            List<PLAN_INVOICE> lstItem = new List<PLAN_INVOICE>();
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        public int addInvoice(List<PLAN_INVOICE> lstItem)
        {
            //1.新增憑證資料
            int i = 0;
            log.Info("add invoice = " + lstItem.Count);
            //2.將扣款資料寫入 
            using (var context = new TopmepEntities())
            {
        ...
        {
            int pieces = 0;
            log.Info("get invoice pieces by form id  =" + estid);
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        {
            string terms = null;
            log.Info("get payment terms by contractid  =" + contractid);
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
...
";
            log.Info("get repayment :" + sql + " by EST id =" + id);
        ...
        public List<RePaymentFunction> getSupplierOfContractByPrjId(string prjid)
        {
            ...
            log.Info("get repayment by projectid  =" + prjid);
            List<RePaymentFunction> lstItem = new List<RePaymentFunction>();
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        public int AddRePay(List<PLAN_OTHER_PAYMENT> lstItem)
        {
            //1.新增代付支出資料
            int i = 0;
            log.Info("add repayment = " + lstItem.Count);
            //2.將代付支出資料寫入 
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_OTHER_PAYMENT item in lstItem)
            ...
            log.Info("add repayment count =" + i);
            return i;
        }

        public int delRePayByESTId(string estid)
        {
            log.Info("remove all repayment detail by EST FORM ID=" + estid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete these repayment record by est form id=" + estid);
        ...
        public List<RePaymentFunction> getRefundById(string id)
        {
            ...
            log.Info("get refund by EST id =" + id);
            List<RePaymentFunction> lstItem = new List<RePaymentFunction>();
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        public List<RePaymentFunction> getRefundOfSupplierById(string contractid)
        {
            ...
            log.Info("get all refunds of this supplier by contractid =" + contractid);
            List<RePaymentFunction> lstItem = new List<RePaymentFunction>();
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        public List<RePaymentFunction> getSupplierOfContractRefundById(string contractid)
        {
            ...
            log.Info("get refund by contractid  =" + contractid);
            List<RePaymentFunction> lstItem = new List<RePaymentFunction>();
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        {
            log.Info("remove all refund detail by EST FORM ID=" + estid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete these refund record by est form id=" + estid);
        ...
        public int RefreshRefund(List<PLAN_OTHER_PAYMENT> lstItem)
        {
            //1.新增代付扣回資料
            int i = 0;
            log.Info("add refund = " + lstItem.Count);
            //2.將代付扣回資料寫入 
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_OTHER_PAYMENT item in lstItem)
        ...
        public decimal getBalanceOfRefundById(string id)
        {
            ...
            log.Info("get the balance of refund by contractid  =" + id);
            decimal balance = 0;
            //處理SQL 預先填入ID,設定集合處理參數
            using (var context = new TopmepEntities())
            {
        ...
        {
            log.Info("remove all invoice detail by EST FORM ID=" + estid);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete these invoice record by est form id=" + estid);
                i = context.Database.ExecuteSqlCommand("DELETE FROM PLAN_INVOICE WHERE EST_FORM_ID=@estid ", new SqlParameter("@estid", estid));
            }
            log.Debug("delete PLAN INVOICE count=" + i);
            return i;
        }
        #endregion
        ...
        public List<CashFlowFunction> getCashFlow()
        {
            ...
            log.Info("get cash flow order by date in six months !!");
...
";
                ...
                log.Info("Get Cash Flow Count=" + lstItem.Count);
            }
        ...
        {
            int i = 0;
            log.Info("refreshExpBudgetItem = " + items.Count);
            //2.將Excel 資料寫入 
            using (var context = new TopmepEntities())
            {
            ...
            log.Info("add FIN_EXPENSE_BUDGET count =" + i);
            return i;
        }
        public int delExpBudgetByYear(int year)
        {
            log.Info("remove all expense budget by budget year=" + year);
            int i = 0;
            using (var context = new TopmepEntities())
            {
                log.Info("delete all FIN_EXPENSE_BUDGET by budget year =" + year);
                i = context.Database.ExecuteSqlCommand("DELETE FROM FIN_EXPENSE_BUDGET WHERE BUDGET_YEAR=@year", new SqlParameter("year", year));
            }
            log.Debug("deleteFIN_EXPENSE_BUDGET count=" + i);
            return i;
        }
        ...
        public List<FIN_SUBJECT> getSubjectByChkItem(string[] lstItemId)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("Get Subject of Expense  Count=" + lstSubject.Count);
            }
            return lstSubject;
        }
        ...
        {
            //1.建立公司營業費用單/工地費用單
            log.Info("create new expense form ");
            string sno_key = "EXP";
            SerialKeyService snoservice = new SerialKeyService();
            form.EXP_FORM_ID = snoservice.getSerialKey(sno_key);
            log.Info("new expense form =" + form.ToString());
            using (var context = new TopmepEntities())
            {
                context.FIN_EXPENSE_FORM.Add(form);
                int i = context.SaveChanges();
                log.Debug("Add form=" + i);
                log.Info("expense form id = " + form.EXP_FORM_ID);
                //if (i > 0) { status = true; };
            }
        ...
        public int AddExpenseItems(List<FIN_EXPENSE_ITEM> lstItem)
        {
            //2.新增費用項目資料
            int j = 0;
            log.Info("add expense items = " + lstItem.Count);
            using (var context = new TopmepEntities())
            {
                //3.將費用項目資料寫入 
        ...
        {
            log.Info("get form : formid=" + expid);
            using (var context = new TopmepEntities())
            {
                //取得費用單檔頭資訊
                ...
                log.Debug("get query year of operating expense:" + formEXP.OCCURRED_YEAR);
                log.Debug("get operating expense item count:" + EXPItem.Count);
                ...
                log.Debug("get query create date of operating expense:" + formEXP.CREATE_DATE);
                //取得工地費用單明細
                if (null != formEXP.PROJECT_ID && formEXP.PROJECT_ID != "")
                {
                    ...
                    log.Debug("get query year of plan site expense:" + formEXP.OCCURRED_YEAR);
                    log.Debug("get plan site expense item count:" + siteEXPItem.Count);
        ...
        {
            log.Info("Update expense form id =" + formid);
            int i = 0;
            int j = 0;
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    i = context.SaveChanges();
                    log.Debug("Update expense form =" + i);
                    log.Info("expense form item = " + lstItem.Count);
                    //2.將item資料寫入 
                    foreach (FIN_EXPENSE_ITEM item in lstItem)
                    {
                        FIN_EXPENSE_ITEM existItem = null;
                        log.Debug("form item id=" + item.EXP_ITEM_ID);
                        ...
                        else
                        {
                            ...
                            log.Info(sql + " ;" + formid + ",fin_subject_id=" + item.FIN_SUBJECT_ID);
                        ...
                        log.Debug("find exist item=" + existItem.FIN_SUBJECT_ID);
                        existItem.AMOUNT = item.AMOUNT;
                        existItem.ITEM_REMARK = item.ITEM_REMARK;
                    ...
                    j = context.SaveChanges();
                    log.Debug("Update expense form item =" + j);
                    return j;
                }
                catch (Exception e)
                {
                    log.Error("update new expense form id fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        {
            int i = 0;
            log.Info("update the status of EXP form by expid" + expid);
        ...
        {
            log.Info("search expense form by " + occurreddate + ", 費用單編號 =" + expid + ", 項目名稱 =" + subjectname + ", 估驗單狀態 =" + status + ", 專案編號 =" + projectid);
        ...
        {
            int i = 0;
            log.Info("reject EXP form by expid" + expid);
        ...
        {
            int i = 0;
            log.Info("Pass EXP form by expid" + expid);
        ...
        {
            int i = 0;
            log.Info("Journal For EXP form by expid" + expid);
        ...
        {
            int i = 0;
            log.Info("Approve EXP form by expid" + expid);
        ...
        public int refreshAccountStatus(List<PLAN_ACCOUNT> lstItem)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("plan account item = " + lstItem.Count);
                //將item資料寫入 
                foreach (PLAN_ACCOUNT item in lstItem)
                {
                    PLAN_ACCOUNT existItem = null;
                    log.Debug("form item id=" + item.PLAN_ACCOUNT_ID);
                    ...
                    else
                    {
                        ...
                        log.Info(sql + " ;" + item.ACCOUNT_FORM_ID);
                    ...
                    log.Debug("find exist item=" + existItem.ACCOUNT_FORM_ID);
                ...
                j = context.SaveChanges();
                log.Debug("Update plan account item =" + j);
            }
            return j;
        }
        ...
        {
            log.Info("search plan account by " + paymentdate + ", 受款人 =" + payee + ", 專案名稱 =" + projectname + ", 帳款類型 =" + accounttype + ", 單據編號 =" + formid);
        ...
        {
            log.Debug("get plan account item by id=" + itemid);
        ...
        public int delPlanAccountItem(string itemid)
        {
            ...
            using (var db = new TopmepEntities())
            {
                var parameters = new List<SqlParameter>();
                parameters.Add(new SqlParameter("itemid", itemid));
                log.Info("Delete PLAN_ACCOUNT =" + sql + ",itemid=" + itemid);
                i = db.Database.ExecuteSqlCommand(sql, parameters.ToArray());
            }
            return i;
        }
        ...
        {
            log.Debug("get loan tranaction by payment date=" + paymentdate + ",and type = " + type);
        ...
        {
            log.Debug("get cash out flow balance by payment date, and payment date =" + paymentDate);
        ...
        {
            log.Debug("get expense budget by payment date, and payment date =" + paymentDate);
            List<ExpenseFormFunction> lstItem = new List<ExpenseFormFunction>();
            using (var context = new TopmepEntities())
            {
                //條件篩選
                if (null != paymentDate && paymentDate != "")
                {
                    ...
                    log.Debug("sql=" + sql + ",paymentdate=" + paymentDate);
                ...
                else if (null != duringStart && duringStart != "" && null != duringEnd && duringEnd != "")
                {
                    ...
                    log.Debug("sql=" + sql + ",duringStart=" + duringStart + ",duringEnd=" + duringEnd);
        ...
        public int updatePlanAccountItem(PLAN_ACCOUNT item)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("updatePlanAcountItem  fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
            }
        ...
        public string refreshVA(string formid, PLAN_VALUATION_FORM form)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("update VA item fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
      ...
      {
          int i = 0;
          log.Info("refresh rentention amount, advance refund and tax amount of VA by form id" + formid);
          ...
          log.Debug("refresh items from Quote sql:" + sql);
          db = new TopmepEntities();
          var parameters = new List<SqlParameter>();
          parameters.Add(new SqlParameter("formid", formid));
      ...
      */
        ...
        public int addPlanAccount(PLAN_ACCOUNT form)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("update plan account item fail:" + e.ToString());
                    log.Error(e.StackTrace);
                    message = e.Message;
                }
        ...
        public void getAllBankLoan(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                try
                {
                    ...
                    log.Debug("get records=" + lstLoans.Count);
                    //將特定專案之貸款封裝供前端頁面調用
                    cashFlowModel.finLoan = lstLoans;
                }
                catch (Exception e)
                {
                    log.Error("fail:" + e.StackTrace);
                }
            }
        }
        ...
        {
            int i = 0;
            log.Info("pay back to loan account by loan id" + loanid);
        ...
        public List<RevenueFromOwner> getVAFileByFormId(string formid)
        {
            ...
            log.Info(" get VA file by form id =" + formid);
            List<RevenueFromOwner> lstItem = new List<RevenueFromOwner>();
            //處理SQL 預先填入專案代號,設定集合處理參數
            string sql = "SELECT f.FILE_ID AS ITEM_UID, f.PROJECT_ID, f.FILE_UPLOAD_NAME, f.FILE_ACTURE_NAME, f.FILE_TYPE, " +
            ...
            using (var context = new TopmepEntities())
            {
                log.Debug("get VA file sql=" + sql);
                lstItem = context.Database.SqlQuery<RevenueFromOwner>(sql, parameters.ToArray()).ToList();
            }
            log.Info("get VA file record count=" + lstItem.Count);
            return lstItem;
        }
        ...
        public List<PlanFinanceProfile> getPlanFinProfile()
        {
            ...
            log.Info("get the finance profile of all plans !! ");
...
;";
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("Get Plan Financial Profile Count=" + lstItem.Count);
            }
        ...
        {
            log.Debug("get credit note by form id, and form id =" + formid);
