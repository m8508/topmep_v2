    ...
    public class ProjectPlanService : ContextService
    {
        ...
        public DataTable getChildTask(string projectid, int prjuid)
        {
            string sql = "WITH PrjTree(TASK_NAME, PRJ_UID, LV_NO,PRJ_ID,PARENT_UID,START_DATE,FINISH_DATE,DURATION) AS "
                ...
                + " FROM PrjTree ORDER BY PRJ_ID";
            log.Debug("sql=" + sql);
            log.Debug("prj_id=" + projectid + ",prjUID=" + prjuid);
        ...
        public PLAN_TASK getRootTask(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                {
                    log.Error("Task Not found!!" + ex.Message);
        ...
        public int setRootTask(string projectid, string prjuid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("sql=" + sql + ",projectid=" + projectid + ",prjuid=" + prjuid);
        ...
        public string getProjectTask4Tree(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("row count=" + lstTask.Count);
            }
            ...
            foreach (PLAN_TASK t in lstTask)
            {
                ...
                if (null == t.PARENT_UID || t.PARENT_UID == 0 || dicTree.Count == 0)
                {
                    ...
                    log.Info("add root node :" + t.PRJ_UID);
                }
                else
                {
                    //將Dic 內的節點翻出，加入子節點
                    ...
                    log.Info("add  node :" + t.PRJ_UID + ",parent=" + t.PRJ_UID);
                }
            }
        ...
        {
            log.Info("get map DEVICE info by item_name=" + item_name);
            string sql = @"SELECT P.* 
                FROM PLAN_ITEM P 
...
";
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Info("Plan Item Not In Map:" + sql);
        ...
        public void getMapLCP(string projectid, string mapno, string buildno, string primeside, string primesidename, string secondside, string secondsidename, string name)
        {
            ...
            {
                log.Debug("item id=" + lstMapLCP[i].ITEM_DESC + ",Qty=" + lstMapLCP[i].ITEM_QUANTITY);
            }
        ...
        {
            int i = -1;
            log.Info("projectid=" + projectid + ",prjuid=" + prjuid + ",mapdeviceIds=" + mapdeviceIds);
        ...
        {
            int i = -1;
            log.Info("projectid=" + projectid + ",prjuid=" + prjuid + ",mapPepIds=" + mapPepIds);
        ...
        {
            int i = -1;
            log.Info("projectid=" + projectid + ",prjuid=" + prjuid + ",mapLcpIds=" + mapLcpIds);
        ...
        {
            int i = -1;
            log.Info("projectid=" + projectid + ",prjuid=" + prjuid + ",mapFpIds=" + mapFpIds);
        ...
        {
            int i = -1;
            log.Info("projectid=" + projectid + ",prjuid=" + prjuid + ",mapPitemIds=" + mapPitemIds);
        ...
        {
            int i = -1;
            log.Info("projectid=" + projectid + ",prjuid=" + prjuid + ",MAP_PLU=" + mappluIds);
        ...
        {
            log.Info("get ItemTask Project_id=" + projectid + ",prjuid=" + prjuid);
...
,[CREATE_ID],[CREATE_DATE],[MODIFY_ID],[MODIFY_DATE]
 ...
 ORDER BY START_DATE;";
                    log.Debug("sql=" + sql);
                    log.Debug("dt" + dt.ToString("yyyy-MM-dd"));
            ...
            log.Info("get task by now:" + lstTask.Count);
            return lstTask;
        }
        //讀取專案基本資料
        ...
        public PLAN_TASK getProjectTask(string projectid, int prjuid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("Task Info=" + planTask.TASK_NAME);
            }
            return planTask;
        }
        //建立新的施工日報資料使用
        ...
        public DailyReport getDailyReport(string reportId)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("get daily report ,sql=" + sql + ",reportId=" + reportId);
                ...
                log.Debug("get notes ,sql=" + sql + ",reportId=" + reportId);
                ...
                try
                {
                    ...
                    log.Debug("Done Task=" + flag.Count);
                    if (flag.Count > 0)
                    {
        ...
        public string createDailyReport(DailyReport dr)
        {
            try
            {
                using (var db = new TopmepEntities())
                {
                    ...
                    log.Info("remove daily Report:" + sql + ",reportid=" + dr.dailyRpt.REPORT_ID);
                    int i = db.Database.ExecuteSqlCommand(sql, new SqlParameter("reportId", dr.dailyRpt.REPORT_ID));
                    log.Debug("remove daily Report:" + i);
                    //2.
        ...
        public List<DailyReportItem> getItem(string projectid, int prjuid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                ...
                log.Debug("lstDailyRptItem count=" + lstDailyRptItem.Count);
            }
...
";
            using (var context = new TopmepEntities())
            {
                log.Debug("sql=" + sql + ",reportId=" + reportId);
        ...
        public List<DailyReportRecord4Worker> getDailyReportRecord4Worker(string projectid, string reportid, string type)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("get DailyReportRecord4Worker sql:" + sql + ",type=" + type);
        ...
        public List<PLAN_DR_TASK> getTaskByReportId(string reportid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("get task for report sql:" + sql);
...
";
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("get getSummaryReport sql:" + sql + ",projectid=" + projectid + ",report date between:" + reportstartdate + "," + reportenddate);
        ...
        public List<SummaryDailyReport> getSummaryReport(string projectid)
        {
            ...
            using (var context = new TopmepEntities())
            {
                log.Info("get getSummaryReport sql:" + sql + ",projectid=" + projectid);
                lst = context.Database.SqlQuery<SummaryDailyReport>(sql, parameters.ToArray()).ToList();
            }
            return lst;
        }
            ...
            using (var context = new TopmepEntities())
            {
                foreach (PLAN_TASK2MAPITEM item in lstTask2Map)
                {
                    if ((item.PROJECT_ITEM_ID != null || item.PROJECT_ITEM_ID != "") && (item.PRJ_UID != 0))
                    {
                        ...
                        log.Debug(sql + "projectId=" + item.PROJECT_ID + ",prjuid=" + item.PRJ_UID + ",maptyp=" + item.MAP_TYPE + ",projectitem_Id=" + item.PROJECT_ITEM_ID);
                        var parameters = new List<SqlParameter>();
                        //設定專案名編號資料
