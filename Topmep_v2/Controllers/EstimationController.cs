...
namespace topmeperp.Controllers
{
    public class EstimationController : Controller
    {
        ...
        {
            log.Info("valuation index : projectid=" + id);
        ...
        {
            log.Info("create EST form id process!  Contract id=" + id);
            //新增估驗單編號
        ...
        public ActionResult createEstimationOrder()
        {
            ...
            log.Debug("get estimation Info :" + contract.supplier.COMPANY_NAME);
        ...
        public ActionResult createEstimationOrderInfo()
        {
            ...
            log.Debug("get estimation order=" + formid);
            EstimationService service = new EstimationService();
        ...
        public string saveEstimationOrder(FormCollection f)
        {
            ...
            log.Debug("check:" + strPaymentTransfer);
            ...
            log.Debug("get form Id=" + form.EST_FORM_ID);
        ...
        public string delEstimationOrder(FormCollection f)
        {
            string formId = f["formId"];
            SYS_USER u = UtilService.GetUserInfoFromSession(Session);
            log.Info(u.USER_ID + " delete estimation form=" + formId);
            EstimationService service = new EstimationService();
        ...
        public ActionResult ContractItems(string projectid)
        {
            ...
            log.Info("Access To Est Form By Project Id =" + projectid);
            string formid = service.getEstNo();
        ...
        {
            //取得估驗單編號
            log.Info("EST form Id:" + Request["formid"]);
            SYS_USER u = (SYS_USER)Session["user"];
            log.Info("update Estimation Form");
            //估驗單草稿 STATUS = 0
        ...
        {
            log.Info("http get mehtod:" + f["EST_FORM_ID"]);
            //取得單據與任務資料
        ...
        public string CancelEstimationForm(FormCollection f)
        {
            Flow4EstimationForm wfs = new Flow4EstimationForm();
            wfs.getTask(f["formId"]);
            log.Info("Cancel EstimationForm:" + wfs.task.task.EST_FORM_ID);
        ...
        public string RejectEstimationForm(FormCollection f)
        {
            Flow4EstimationForm wfs = new Flow4EstimationForm();
            wfs.getTask(f["formId"]);
            log.Info("Reject EstimationForm:" + wfs.task.task.EST_FORM_ID);
            SYS_USER u = (SYS_USER)Session["user"];
        ...
        public ActionResult AddEst(FormCollection f)
        {
            //取得估驗單編號
            log.Info("EST form Id:" + Request["formid"]);
            log.Info("get EST No of un Approval By contractid:" + Request["contractid"]);
        ...
        public String ConfirmEst(PLAN_ESTIMATION_FORM est, HttpPostedFileBase file)
        {
            ...
            {
                log.Error(est.PAYMENT_TRANSFER + " not payment_transfer:" + ex.Message);
            }
            try
            {
            ...
            {
                log.Error(est.PAID_AMOUNT + " not paid_amount:" + ex.Message);
            }
            //若使用者有上傳計價附檔，則增加檔案資料
            if (null != file && file.ContentLength != 0)
            {
                ...
                //2.解析檔案
                log.Info("Parser file data:" + file.FileName);
                //2.1 設定檔案名稱,實體位址,附檔名
                string projectid = Request["id"];
                string keyName = Request["estid"];
                log.Info("file upload namme =" + keyName);
                ...
                //2.2 將上傳檔案存檔
                log.Info("save upload file:" + path);
                file.SaveAs(path);
            }
            ...
            flowService.iniRequest(u, estid);
            ...
            log.Info("EST form details payment amount=" + itemJson);
            return itemJson;
            //}
        }
        //估驗單查詢
        ...
        public ActionResult SingleEST(string id)
        {
            ...
            log.Debug("Project ID:" + singleForm.project.PROJECT_ID);
            ...
            log.Info("get process request by dataId=" + id);
            wfs.getTask(id);
            wfs.getRequest(id);
        ...
        public String SendForm(FormCollection f)
        {
            ...
            log.Info("Data In Session :" + wfs.task.EstData.planEST.EST_FORM_ID);
        ...
        {
            log.Info("Access To Other Payment By EST Form Id =" + id);
            ...
            log.Debug("this other payment record =" + ViewBag.key + "筆");
        ...
        public String UpdateOtherPay(FormCollection form)
        {
            ...
            string msg = "";
            // 先刪除原先資料
            log.Info("EST form id =" + form["formid"]);
            log.Info("Delete PLAN_OTHER_PAYMENT By EST_FORM_ID");
            ...
            for (int j = 0; j < lstAmount.Count(); j++)
            {
                ...
                log.Info("Other Payment Amount  =" + item.AMOUNT);
                item.REASON = lstReason[j];
                log.Debug("Item EST form id =" + item.EST_FORM_ID + "且扣款原因為" + item.REASON);
                lstItem.Add(item);
            }
        ...
        */
        ...
        {
            log.Info("Access To Advance Payment By EST Form Id =" + id);
        ...
        public String AddAdvancePay(FormCollection form)
        {
            ...
            for (int j = 0; j < lstItemId.Count(); j++)
            {
                ...
                log.Info("Advance Payment Amount  =" + item.AMOUNT);
                item.TYPE = lsttype[j];
                log.Debug("Item EST form id =" + item.EST_FORM_ID + " ,And Type =" + item.TYPE);
                lstItem.Add(item);
            }
       ...
       */
        ...
        {
            log.Info("Access To Invoice By EST Form Id =" + id);
            service.getInqueryForm(contractid);
            PLAN_SUP_INQUIRY f = service.formInquiry;
            ViewBag.projectId = f.PROJECT_ID;
            TND_PROJECT p = service.getProjectById(f.PROJECT_ID);
            ...
            log.Debug("this invoice record =" + ViewBag.key + "筆");
        ...
        public String AddInvoice(FormCollection form)
        {
            ...
            for (int j = 0; j < lstAmount.Count(); j++)
            {
                ...
                log.Info("Invoice Number = " + item.INVOICE_NUMBER + "and Invoice Amount =" + item.AMOUNT);
        ...
        public String UpdateInvoice(FormCollection form)
        {
            ...
            // 先刪除原先資料
            log.Info("EST form id =" + form["formid"]);
            log.Info("Delete PLAN_INVOICE By EST_FORM_ID");
        ...
        public ActionResult ChooseSupplier(string id, string contractid)
        {
            ...
            log.Debug("supplier record =" + ViewBag.key + "筆");
        ...
        public ActionResult AddRePayment(FormCollection form)
        {
            ...
            for (int j = 0; j < lstItemId.Count(); j++)
            {
                ...
                log.Debug("Item EST form id =" + item.EST_FORM_ID + " ,and supplier for refund =" + item.CONTRACT_ID_FOR_REFUND);
                lstItem.Add(item);
            }
        ...
        public String UpdateRePay(FormCollection form)
        {
            ...
            for (int j = 0; j < lstAmount.Count(); j++)
            {
                ...
                log.Debug("Item EST form id =" + item.EST_FORM_ID + "且扣款原因為" + item.REASON + "且代付支出對象為" + item.CONTRACT_ID_FOR_REFUND);
                lstItem.Add(item);
            }
        ...
        {
            log.Info("Access To Refund By EST Form Id =" + id);
            service.getInqueryForm(contractid);
            PLAN_SUP_INQUIRY f = service.formInquiry;
            ViewBag.projectId = f.PROJECT_ID;
            TND_PROJECT p = service.getProjectById(f.PROJECT_ID);
        ...
        public String UpdateRefund(FormCollection form)
        {
            ...
            for (int j = 0; j < lstAmount.Count(); j++)
            {
                ...
                log.Debug("Item EST form id =" + item.EST_FORM_ID + "且扣款原因為" + item.REASON + "且請款對象為" + item.CONTRACT_ID_FOR_REFUND + "且請款單號為" + item.EST_FORM_ID_REFUND);
                lstItem.Add(item);
            }
        ...
        {
            log.Debug("get EST No of un Approval By contractid:" + Request["contractid"]);
            string contractid = Request["contractid"];
            string UnApproval = null;
            UnApproval = service.getEstNoByContractId(contractid);
            return UnApproval;
        ...
        {
            log.Info("Access to Expense Page !!");
            ViewBag.projectid = id;
            TND_PROJECT p = service.getProjectById(id);
            ViewBag.projectName = p.PROJECT_NAME;
        ...
        {
            //取得使用者勾選品項ID
            log.Info("item_list:" + Request["subject"]);
        ...
        public ActionResult AddExpense(FIN_EXPENSE_FORM ef)
        {
            ...
            //建立工地費用單號
            log.Info("create new Plan Expense Form");
            UserService us = new UserService();
            SYS_USER u = (SYS_USER)Session["user"];
            ...
            for (int j = 0; j < lstSubject.Count(); j++)
            {
                ...
                log.Info("Plan Expense Subject =" + item.FIN_SUBJECT_ID + "， and Amount = " + item.AMOUNT);
                item.EXP_FORM_ID = fid;
                log.Debug("Item EX form id =" + item.EXP_FORM_ID);
                lstItem.Add(item);
            }
            int i = service.AddExpenseItems(lstItem);
            //建立申請單參考流程
            Flow4SiteExpense flowService = new Flow4SiteExpense();
            log.Debug("Item Count =" + i);
        ...
        {
            log.Info("Access to Site Budget Page, And Project Id = " + id);
            ViewBag.projectid = id;
            TND_PROJECT p = service.getProjectById(id);
            ViewBag.projectName = p.PROJECT_NAME;
            //取得工地費用預算資料
        ...
        public String UpdateSiteBudgetOfFirstYear(FormCollection form)
        {
            log.Info("form:" + form.Count);
            // 先刪除原先資料
            log.Info("Site Expense Budget's Project Id =" + form["projectId"]);
            log.Info("Delete PLAN_SITE_BUDGET By PROJECT_ID and YEAR_SEQUENCE");
            var year = 1.ToString();
            ...
            for (int j = 0; j < lstsubjctid.Count(); j++)
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    ...
                    log.Info("費用項目代碼 =" + item.SUBJECT_ID + "，and Budget Month = " + item.BUDGET_MONTH + "，and Atm = " + Atm[i][j]);
                    lstItem.Add(item);
                }
            ...
            log.Info("Request:BUDGET_YEAR =" + form["firstYear"]);
            return msg;
        }
        #endregion
        #region 第2年度
        public String UpdateSiteBudgetOfSecondYear(FormCollection form)
        {
            ...
            log.Info("Request:BUDGET_YEAR =" + form["secondYear"]);
            return msg;
        }
        #endregion
        #region 第3年度
        public String UpdateSiteBudgetOfThirdYear(FormCollection form)
        {
            ...
            log.Info("Request:BUDGET_YEAR =" + form["thirdYear"]);
            return msg;
        }
        #endregion
        #region 第4年度
        public String UpdateSiteBudgetOfFourthYear(FormCollection form)
        {
            ...
            log.Info("Request:BUDGET_YEAR =" + form["fourthYear"]);
            return msg;
        }
        #endregion
        #region 第5年度
        public String UpdateSiteBudgetOfFifthYear(FormCollection form)
        {
            ...
            log.Info("Request:BUDGET_YEAR =" + form["fifthYear"]);
            return msg;
        }
        #endregion
            ...
            HttpPostedFileBase fileBudget4, HttpPostedFileBase fileBudget5)
        {
            string projectid = Request["projectid"];
            string message = "";
            log.Info("Upload plan Budget of Site Expenses for projectid=" + projectid);
            try
            {
                //檔案變數名稱需要與前端畫面對應
                #region 預算第1年度
                ...
                {
                    //2.解析Excel
                    log.Info("Parser Excel data:" + fileBudget1.FileName);
                    //2.1 設定Excel 檔案名稱
                    ...
                    //2.2 開啟Excel 檔案
                    log.Info("Parser Excel File Begin:" + fileBudget1.FileName);
                    ...
                    //2.4
                    var year = 1.ToString();
                    log.Info("Delete PALN_SITE_BUDGET By Project");
                    service.delSiteBudgetByProject(projectid, year);
                    message = message + "<br/>舊有資料刪除成功 !!";
                    //2.5 
                    log.Info("Add First Year PALN_SITE_BUDGET to DB");
                ...
                if (null != fileBudget2 && fileBudget2.ContentLength != 0)
                {
                    ...
                    log.Info("Parser Excel data:" + fileBudget2.FileName);
                    //2.1 設定Excel 檔案名稱
                    ...
                    //2.2 開啟Excel 檔案
                    log.Info("Parser Excel File Begin:" + fileBudget2.FileName);
                    SiteBudgetFormToExcel budgetservice = new SiteBudgetFormToExcel();
                    budgetservice.InitializeWorkbook(path);
                    //解析預算數量
                    ...
                    message = message + "<br/>舊有資料刪除成功 !!";
                    //2.5 
                    log.Info("Add Second Year PALN_SITE_BUDGET to DB");
                ...
                {
                    //2.解析Excel
                    log.Info("Parser Excel data:" + fileBudget3.FileName);
                    //2.1 設定Excel 檔案名稱
                    ...
                    //2.2 開啟Excel 檔案
                    log.Info("Parser Excel File Begin:" + fileBudget3.FileName);
                    SiteBudgetFormToExcel budgetservice = new SiteBudgetFormToExcel();
                    budgetservice.InitializeWorkbook(path);
                    //解析預算數量
                ...
                {
                    //2.解析Excel
                    log.Info("Parser Excel data:" + fileBudget4.FileName);
                    //2.1 設定Excel 檔案名稱
                    ...
                    //2.2 開啟Excel 檔案
                    log.Info("Parser Excel File Begin:" + fileBudget4.FileName);
                    SiteBudgetFormToExcel budgetservice = new SiteBudgetFormToExcel();
                    budgetservice.InitializeWorkbook(path);
                    //解析預算數量
                ...
                {
                    //2.解析Excel
                    log.Info("Parser Excel data:" + fileBudget5.FileName);
                    //2.1 設定Excel 檔案名稱
                    ...
                    //2.2 開啟Excel 檔案
                    log.Info("Parser Excel File Begin:" + fileBudget5.FileName);
                    SiteBudgetFormToExcel budgetservice = new SiteBudgetFormToExcel();
                    budgetservice.InitializeWorkbook(path);
                    //解析預算數量
                    ...
                    //2.4
                    var year = 5.ToString();
                    log.Info("Delete PLAN_SITE_BUDGET By Project");
                    service.delSiteBudgetByProject(projectid, year);
                    message = message + "<br/>舊有資料刪除成功 !!";
                    //2.5 
                    log.Info("Add Second Year PALN_SITE_BUDGET to DB");
        ...
        {
            log.Info("Access to Site Expense and Budget Summary Page，Project Id =" + id);
            ViewBag.projectid = id;
            TND_PROJECT p = service.getProjectById(id);
            ViewBag.projectName = p.PROJECT_NAME;
            //取得工地每年預算資料
        ...
        {
            log.Info("Access to Site Expense and Budget Summary Page，Project Id =" + Request["projectid"]);
            string projectId = Request["projectid"];
        ...
        {
            log.Info("Access to Valuation For Owner Page，Project Id =" + id);
            ViewBag.projectid = id;
            TND_PROJECT p = service.getProjectById(id);
            ViewBag.projectName = p.PROJECT_NAME;
            //取得付款條件
        ...
        {
            log.Info("create valuation form process! project =" + Request["projectId"]);
        ...
        {
            log.Info("get AR of VA form by form id=" + formid);
            System.Web.Script.Serialization.JavaScriptSerializer objSerializer = new System.Web.Script.Serialization.JavaScriptSerializer();
            string itemJson = objSerializer.Serialize(service.getVADetailByVAId(formid));
            log.Info("VA AR's Info=" + itemJson);
            return itemJson;
        }
        //新增應收帳款支付資料
        public String addPaymentDate(FormCollection form)
        {
            ...
            if (null != form.Get("loans").Trim() && form.Get("loans").Trim() != "")
            {
                ...
                log.Info("paybackAtm = " + paybackAtm);
            }
            if (form["fee"] != "")
            {
            ...
            {
                int k = service.addLoanTransaction(int.Parse(form.Get("loans").Trim()), paybackAtm, paybackDate, u.USER_ID, period, loanRemark, form["va_form_id"]);
            }
        ...
        {
            log.Info("Access to the Page abount files of Valuation!!");
            List<RevenueFromOwner> lstItem = service.getVAFileByFormId(id);
            log.Debug("va_form_id = " + id);
            return View(lstItem);
        }
        /// <summary>
        ///  業主計價附檔
        ...
        public String delVAFile()
        {
            ...
            log.Info(loginUser.USER_ID + " remove data:va file uid=" + itemUid);
            FileManage fs = new FileManage();
            TND_FILE f = fs.getFileByItemId(long.Parse(Request["itemid"]));
